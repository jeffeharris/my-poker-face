...
export function PersonalityManager({ onBack, embedded = false }: PersonalityManagerProps) {
  ...
  const editorSections = selectedName && formData ? (
    <div className="pm-sections">
        ...
        {/* Staking Profile (admin only) — borrower side: do they
            accept stakes, and at what trust threshold. Lender side
            still lives in the seed JSON for now (no UI). */}
        {isAdmin && (
          <CollapsibleSection
            title="Staking Profile"
            icon={<Coins size={20} />}
