...
interface WebhookSetting extends SettingConfig {
  configured?: boolean;
  sensitive?: boolean;
}
...
export function UnifiedSettings({
  ...
  // Alerting (webhook) state. The URL is a secret — we only ever hold the
  // masked value from the server; the input is what the admin is setting.
  ...
  useEffect(() => {
    if (activeCategory === 'models' && !systemSettings) {
      ...
      fetchStorage();
    } else if (activeCategory === 'alerting' && !alertingSetting) {
      fetchAlertingData();
    }
