...
class TestBadBeatFires:
    def test_river_runout_kills_favorite(self):
        ...
        bad_beats = [e for e in events if e.event is RelationshipEvent.BAD_BEAT]
...
class TestBadBeatDedupAndIds:
    def test_dedup_blocks_double_emit(self):
        ...
        assert any(e.event is RelationshipEvent.BAD_BEAT for e in first)
        assert second == []
    ...
    def test_ids_resolved_from_registry(self):
        ...
        bb = next(e for e in events if e.event is RelationshipEvent.BAD_BEAT)
