...
class TestExperimentConfigVariants:
    ...
    def test_get_variant_configs_legacy_mode(self):
        ...
        assert label is None  # No label in legacy mode
        assert effective_config['model'] == 'gpt-5-nano'
    ...
    def test_get_variant_configs_control_with_variants(self):
        ...
        label, effective_config = variants[0]
        assert label == 'GPT Baseline'
        assert effective_config['model'] == 'gpt-5-nano'
        assert effective_config['provider'] == 'openai'

        # Second should be first variant (overrides model/provider)
        label, effective_config = variants[1]
        assert label == 'Claude Sonnet'
        assert effective_config['model'] == 'claude-sonnet-4-20250514'
        assert effective_config['provider'] == 'anthropic'

        # Third should be second variant (overrides model/provider)
        label, effective_config = variants[2]
        assert label == 'Claude Haiku'
        assert effective_config['model'] == 'claude-haiku-4-5-20251001'
        assert effective_config['provider'] == 'anthropic'
    ...
    def test_get_variant_configs_variant_inherits_from_experiment(self):
        ...
        assert control_config['model'] == 'gpt-5-nano'
        assert control_config['provider'] == 'openai'

        # Variant inherits model and provider from experiment (not control)
        label, variant_config = variants[1]
        assert label == 'No Pot Odds'
        assert variant_config['model'] == 'gpt-5-nano'
        assert variant_config['provider'] == 'openai'
    ...
    def test_get_total_tournaments_legacy_mode(self):
        ...
        assert config.get_total_tournaments() == 5
    ...
    def test_get_total_tournaments_with_variants(self):
        ...
        # 3 tournaments per variant x 3 variants (control + 2) = 9
        assert config.get_total_tournaments() == 9
    ...
    def test_get_total_tournaments_control_only(self):
        ...
        # 5 tournaments x 1 variant (control only) = 5
        assert config.get_total_tournaments() == 5


class TestBuildEffectiveVariantConfigGameMode:
    ...
    def test_build_effective_with_game_mode(self):
        ...
        assert result['game_mode'] == 'pro'  # Variant overrides control
    ...
    def test_game_mode_inheritance_from_control(self):
        ...
        assert result['game_mode'] == 'standard'
    ...
    def test_variant_config_dataclass_game_mode(self):
        ...
        assert config.game_mode == "pro"
        d = config.to_dict()
        assert d['game_mode'] == "pro"
    ...
    def test_control_config_dataclass_game_mode(self):
        ...
        assert config.game_mode == "standard"
        d = config.to_dict()
        assert d['game_mode'] == "standard"
    ...
    def test_variant_config_from_dict_with_game_mode(self):
        ...
        assert config.game_mode == 'casual'
...
class TestGameModeInVariants:
    ...
    def test_game_mode_in_variant_config(self):
        ...
        # Control
        label, control_config = variants[0]
        assert control_config['game_mode'] == 'casual'
        # Variant
        label, variant_config = variants[1]
        assert variant_config['game_mode'] == 'pro'
    ...
    def test_game_mode_inheritance_from_control(self):
        ...
        label, variant_config = variants[1]
        assert variant_config['game_mode'] == 'standard'
    ...
    def test_game_mode_none_when_not_specified(self):
        ...
        label, control_config = variants[0]
        assert control_config['game_mode'] is None
        label, variant_config = variants[1]
        assert variant_config['game_mode'] is None

    def test_invalid_game_mode_in_control_raises(self):
        """Invalid game_mode in control should raise ValueError."""
        with pytest.raises(ValueError, match='Invalid control game_mode'):
            ExperimentConfig(
                name='test_invalid',
    ...
    def test_invalid_game_mode_in_variant_raises(self):
        ...
        with pytest.raises(ValueError, match=r'Invalid variants\[0\] game_mode'):
            ExperimentConfig(
                name='test_invalid',
                control={'label': 'Control'},
    ...
    def test_valid_game_modes_accepted(self):
        ...
        variants = config.get_variant_configs()
        assert len(variants) == 4  # control + 3 variants


class TestTournamentResultVariant:
    ...
    def test_tournament_result_with_variant(self):
        ...
        assert result.variant == 'Claude Sonnet'


class TestExperimentRoutesVariantValidation:
    ...
    def test_validate_config_control_missing_label(self):
        ...
        with patch('flask_app.routes.experiment_routes.persistence', self.persistence):
            ...
            assert 'control.label is required' in data['errors']
    ...
    def test_validate_config_variant_missing_label(self):
        ...
        with patch('flask_app.routes.experiment_routes.persistence', self.persistence):
            ...
            assert response.status_code == 200
            assert data['valid'] is False
            assert 'variants[0].label is required' in data['errors']
    ...
    def test_validate_config_duplicate_variant_labels(self):
        ...
        with patch('flask_app.routes.experiment_routes.persistence', self.persistence):
            ...
            assert response.status_code == 200
            assert data['valid'] is False
            assert any('Duplicate variant label' in e for e in data['errors'])
    ...
    def test_validate_config_variant_label_matches_control(self):
        ...
        with patch('flask_app.routes.experiment_routes.persistence', self.persistence):
            ...
            assert response.status_code == 200
            assert data['valid'] is False
            assert any('cannot match control label' in e for e in data['errors'])
    ...
    def test_validate_config_warns_total_tournaments_exceeds_20(self):
        ...
        with patch('flask_app.routes.experiment_routes.persistence', self.persistence):
            ...
            assert response.status_code == 200
            assert data['valid'] is True
            # 10 x 3 = 30 tournaments
            assert any('30' in w and 'exceeds 20' in w for w in data['warnings'])
