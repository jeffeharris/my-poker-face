...
class TestLookupBasic:
    def test_premium_hand_jams_at_all_depths(self):
        for depth in [5, 7, 10, 12, 15]:
            result = push_fold.lookup_push_fold_action(
                hand='AA', position='SB', effective_stack_bb=depth,
            )
    ...
    def test_trash_hand_folds_at_all_depths(self):
        for depth in [5, 7, 10, 12, 15]:
            result = push_fold.lookup_push_fold_action(
                hand='32o', position='SB', effective_stack_bb=depth,
            )
    ...
    def test_range_widens_as_depth_shrinks(self):
        ...
        result_15 = push_fold.lookup_push_fold_action(
            hand='76s', position='SB', effective_stack_bb=15,
        )
        result_5 = push_fold.lookup_push_fold_action(
            hand='76s', position='SB', effective_stack_bb=5,
        )
    ...
    def test_returns_none_for_multi_way(self):
        result = push_fold.lookup_push_fold_action(
            hand='AA', position='SB', effective_stack_bb=10,
            num_opponents=3,
        )
    ...
    def test_returns_none_above_threshold(self):
        ...
        result = push_fold.lookup_push_fold_action(
            hand='AA', position='SB', effective_stack_bb=20,
        )
        assert result is None

    def test_returns_none_for_unsupported_position(self):
        result = push_fold.lookup_push_fold_action(
            hand='AA', position='UTG', effective_stack_bb=10,
        )
        assert result is None
...
class TestBBVsJam:
    def test_premium_calls_at_all_depths(self):
        for depth in [5, 7, 10, 12, 15]:
            result = push_fold.lookup_push_fold_action(
                hand='AA', position='BB', effective_stack_bb=depth,
                facing_jam=True,
            )
    ...
    def test_trash_folds_to_jam(self):
        for depth in [5, 7, 10, 12, 15]:
            result = push_fold.lookup_push_fold_action(
                hand='72o', position='BB', effective_stack_bb=depth,
                facing_jam=True,
            )
            if depth >= 10:
    ...
    def test_bb_without_facing_jam_returns_none(self):
        ...
        result = push_fold.lookup_push_fold_action(
            hand='AA', position='BB', effective_stack_bb=10, facing_jam=False,
        )
        assert result is None
    ...
    def test_call_range_tighter_than_push_range(self):
        ...
        sb_action_15 = push_fold.lookup_push_fold_action(
            hand='K6s', position='SB', effective_stack_bb=15,
        )
        bb_action_15 = push_fold.lookup_push_fold_action(
            hand='K6s', position='BB', effective_stack_bb=15, facing_jam=True,
        )
...
class TestDepthSnapping:
    def test_below_min_bucket_clamps_to_min(self):
        ...
        result = push_fold.lookup_push_fold_action(
            hand='AA', position='SB', effective_stack_bb=3,
        )
    ...
    def test_just_above_threshold_returns_none(self):
        result = push_fold.lookup_push_fold_action(
            hand='AA', position='SB', effective_stack_bb=15.5,
        )
        assert result is None
    ...
    def test_intermediate_depth_picks_nearest_bucket(self):
        ...
        result = push_fold.lookup_push_fold_action(
            hand='AA', position='SB', effective_stack_bb=11,
        )
        assert result == 'jam'
...
class TestAggregateBands:
    ...
    CHART_PATH = Path(__file__).resolve().parent.parent.parent / "poker" / "strategy" / "data" / "push_fold_hu.json"
    ...
    @pytest.mark.parametrize("depth,min_pct,max_pct", [
        (5, 70, 90),
        (7, 50, 65),
        (10, 40, 50),
        (12, 30, 40),
    ...
    def test_sb_push_rate_in_band(self, chart, depth, min_pct, max_pct):
        ...
        push_count = sum(
            1 for h, actions in scenario.items()
            if actions.get("jam", 0.0) > 0.5
        )
