...
def test_kl_within_budget():
    ...
    for name, profile in DEVIATION_PROFILES.items():
        result, _trace = modify_strategy(BASE_STRATEGY, BASE_ACTIONS, anchors, emotional, profile)
        result_arr = np.array([result.action_probabilities[a] for a in BASE_ACTIONS])
        kl = _kl_divergence(result_arr, base_arr)
        assert (
            kl <= profile.max_kl + 1e-6
...
def test_per_action_cap_holds():
    anchors = _make_anchors(
        baseline_aggression=0.95,
        baseline_looseness=0.95,
        ego=0.95,
        poise=0.05,
        risk_identity=0.95,
    )
...
def test_no_negative_probabilities():
    ...
    anchors = _make_anchors(
        baseline_aggression=0.0,
        baseline_looseness=0.0,
        ego=1.0,
        poise=0.0,
        risk_identity=0.0,
    )
...
def test_composed_emotional_no_effect():
    ...
    for action in BASE_PROBS:
        assert abs(result.action_probabilities[action] - BASE_PROBS[action]) < 1e-6, (
