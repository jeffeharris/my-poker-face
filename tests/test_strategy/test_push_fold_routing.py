...
def _player(name, stack=1000, bet=0, folded=False):
    return SimpleNamespace(
        name=name, stack=stack, bet=bet, is_folded=folded,
    )
...
def _hu_state(hero_pos='SB', hero_stack_bb=10, hand_open=True,
              big_blind=100):
    """2-handed state. hero_pos in {'SB','BB'}. SB=idx0, BB=idx1.

    When hero is BB and hand_open is True, the SB is all-in (jam to face)."""
    bb = big_blind
    sb = _player('Hero' if hero_pos == 'SB' else 'Villain',
                 stack=hero_stack_bb * bb, bet=bb // 2)
    bb_p = _player('Hero' if hero_pos == 'BB' else 'Villain',
                   stack=hero_stack_bb * bb, bet=bb)
...
_POS_KEY = {
    'UTG': 'under_the_gun', 'HJ': 'middle_position_1', 'CO': 'cutoff',
    'BTN': 'button', 'SB': 'small_blind_player', 'BB': 'big_blind_player',
}


def _6max_state(hero_pos='UTG', hero_idx=0, hero_stack_bb=10, big_blind=100,
                jammer_pos=None, raises=0, num_players=6):
...
class TestMultiwayRouting:
    ...
    def test_6max_three_handed_routes_to_6max(self):
        gs = _6max_state(hero_pos='BTN', hero_idx=0, hero_stack_bb=8,
                         num_players=3)
        c = _controller()
    ...
    def test_6max_facing_sb_jam_calls(self):
        ...
        gs = _6max_state(hero_pos='BB', hero_idx=0, hero_stack_bb=8,
                         jammer_pos='SB')
        c = _controller()
        action = c._try_push_fold_lookup('AA', gs, player_idx=0, num_seated=6)
        assert action == 'call'
...
class TestSnapshotInterplay:
    def test_hu_and_6max_dispatch_distinct(self):
        ...
        hu = c._try_push_fold_lookup(
            'A8o', _hu_state(hero_pos='SB', hero_stack_bb=12),
            player_idx=0, num_seated=2,
        )
        six = c._try_push_fold_lookup(
            'A8o', _6max_state(hero_pos='UTG', hero_idx=0, hero_stack_bb=12),
            player_idx=0, num_seated=6,
        )
        assert hu == 'jam'
        assert six == 'fold'
