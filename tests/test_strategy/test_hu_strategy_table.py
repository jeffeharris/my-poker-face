...
def _row_for(table, *, hand: str, position: str, scenario: str, opener_position: str) -> Dict[str, float]:
    node = PreflopNode(
...
class TestLoadingAndCoverage:
    ...
    def test_all_169_hands_present(self, hu_table, scenario, position, opener):
        ...
        for hand in CANONICAL_HANDS:
            node = PreflopNode(
                hand=hand, position=position,
                scenario=scenario, opener_position=opener,
            )
            if hu_table.lookup_preflop(node) is None:
                missing.append(hand)
        assert not missing, (
            f"{scenario}.{position}_vs_{opener or 'NONE'}: missing hands={missing}"
        )
...
class TestRowsSumToOne:
    ...
    def test_every_row_sums_to_one(self, hu_table, scenario, position, opener):
        ...
        for hand in CANONICAL_HANDS:
            row = _row_for(
                hu_table, hand=hand, position=position,
                scenario=scenario, opener_position=opener,
            )
...
class TestPremiumsAndTrash:
    ...
    def test_trash_offsuit_folds_from_sb(self, hu_table, hand):
        ...
        assert fold_prob >= 0.95, (
            f"{hand} SB fold prob = {fold_prob} (expected >= 0.95); row={row}"
        )
...
class TestAggregateRangeBands:
    def test_sb_open_rate(self, hu_table):
        ...
        assert 0.60 <= rate <= 0.72, (
            f"SB open rate = {rate:.4f}, expected 0.60-0.72"
        )
    ...
    def test_sb_4bet_jam_rate_vs_3bet(self, hu_table):
        ...
        assert 0.06 <= rate <= 0.10, (
            f"SB 4-bet+jam rate vs 3-bet = {rate:.4f}, expected 0.06-0.10"
        )
