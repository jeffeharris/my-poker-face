...
class _StubController:
    ...
    def __init__(self, with_repo: bool = True, with_expression: bool = False):
        ...
        self._persist_decision_analysis = (
            TieredBotController._persist_decision_analysis.__get__(self)
        )
        self._run_expression_layer = (
            TieredBotController._run_expression_layer.__get__(self)
        )
        self._attach_expression = (
            TieredBotController._attach_expression.__get__(self)
        )
...
class TestPersistDirectCall:
    def test_persist_with_capture_id_none_invokes_analyze(self):
        ...
        ctrl._persist_decision_analysis(
            {'action': 'call'}, _fake_game_state(), player_idx=0,
            capture_id=None,
        )
