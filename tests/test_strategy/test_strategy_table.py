...
def _pf_node(spr_bucket, *, texture='dry_high', facing='unopened'):
    return PostflopNode(
        street='flop', position='IP', pot_type='SRP', board_texture=texture,
        made_tier='nuts', draw_modifier='no_draw', facing_action=facing,
...
class TestPostflopSPRFallback:
    ...
    def test_exact_high_lookup(self, pf_table):
        out = pf_table.lookup_postflop_with_fallback(
            _pf_node('high'), ['check', 'raise'])
