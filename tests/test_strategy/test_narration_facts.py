...
_CTX_FLOP = NarrationContext(
    street='flop', position_context='in_position', risk_posture='balanced',
)


def _fire_trace(
    layer: str, rule_id: str = 'default', *,
    ...
    return InterventionTrace(
        layer=layer, rule_id=rule_id, layer_order=layer_order,
        fired=True,
        operation=operation,
...
class TestAllowlistFiltering:
    ...
    def test_no_op_trace_suppressed(self):
        traces = [make_no_op_trace(
            ...
            layer_order=3, reason_code='hand_class_not_eligible',
        )]
        result = traces_to_narration_facts(traces, _CTX_FLOP)
        assert result.facts == []
        assert result.suppressed_facts_count == 1
...
class TestTopNCap:
    def test_caps_to_3_facts(self):
        ...
        traces = [
            _fire_trace('exploitation', 'hyper_aggressive',
                        reason_code='extreme_tier_via_all_in_frequency'),
            _fire_trace('exploitation', 'hyper_passive',
                        reason_code='station_value_extract'),
            _fire_trace('exploitation', 'tight_nit',
                        reason_code='nit_steal_open'),
            _fire_trace('exploitation', 'high_fold_to_cbet',
                        reason_code='hu_cbet_exploit'),
            _fire_trace('bluff_catch_override', 'default',
                        operation=InterventionOperation.OVERRIDE.value,
                        reason_code='medium_made_vs_extreme_facing_bet',
                        layer_order=3),
        ]
    ...
    def test_overridden_layer_downranked(self):
        ...
        traces = [
            _fire_trace('exploitation', 'hyper_aggressive',
                        operation=InterventionOperation.ADJUST.value,
                        reason_code='extreme_tier_via_all_in_frequency',
                        layer_order=1, primary_action_after='call'),
            _fire_trace('bluff_catch_override', 'default',
                        operation=InterventionOperation.OVERRIDE.value,
                        reason_code='medium_made_vs_extreme_facing_bet',
                        layer_order=3, primary_action_after='call',
        ...
        if len(result.facts) >= 2:
            ...
            assert (
                scores['bluff_catch_override'] > scores.get('exploitation', 0)
            )
...
class TestBuckets:
    ...
    def test_intensity_and_certainty_independent(self):
        ...
        traces = [_fire_trace(
            ...
            operation=InterventionOperation.OVERRIDE.value,
            reason_code='medium_made_vs_extreme_facing_bet',
            effect_size=0.8, confidence=0.3,
            layer_order=3,
        )]
        result = traces_to_narration_facts(traces, _CTX_FLOP)
        fact = result.facts[0]
...
class TestScoreFactImportance:
    def test_override_higher_than_adjust(self):
        ctx = _CTX_FLOP
        override = _fire_trace('bluff_catch_override', 'default',
            operation=InterventionOperation.OVERRIDE.value,
            layer_order=3, action_changed=True)
        adjust = _fire_trace('exploitation', 'hyper_aggressive',
            operation=InterventionOperation.ADJUST.value,
            layer_order=1, action_changed=False)
        assert _score_fact_importance(override, ctx, False) > \
               _score_fact_importance(adjust, ctx, False)

    def test_overridden_flag_downranks_0_3x(self):
        ctx = _CTX_FLOP
        trace = _fire_trace('exploitation', 'hyper_aggressive',
            operation=InterventionOperation.ADJUST.value,
            layer_order=1)
