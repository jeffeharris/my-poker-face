...
def _combo_pct(row: dict, action: str) -> float:
    ...
    hit = sum(_hand_combos(h) for h, actions in row.items() if actions.get(action, 0.0) > 0.5)
...
class TestAggregateBands:
    ...
    def test_unopened_jam_pct_in_band(self, chart, pos, depth):
        ...
        assert abs(pct - target) <= self.TOLERANCE, (
            f"{pos} {depth}BB jam {pct:.1f}% vs target {target}% " f"(tol {self.TOLERANCE})"
        )
    ...
    def test_jam_pct_widens_utg_to_sb_at_fixed_depth(self, chart):
        ...
        for d in DEPTHS:
            pcts = [_combo_pct(chart["unopened"][pos][str(d)], "jam") for pos in POSITIONS]
