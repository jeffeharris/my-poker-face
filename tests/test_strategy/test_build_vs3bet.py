...
def built():
    ...
    nodes = {nn: b3.build_node(*nn.split("_vs_"), rfi, vs_open, matrix)
...
def test_fold_to_3bet_under_ceiling_every_node(built):
    ...
    for nn, node in nodes.items():
        ...
        assert f3b <= ceiling + 1e-9, f"{nn}: fold-to-3bet {100*f3b:.1f}% > ceiling {100*ceiling:.0f}%"
    ...
    for nn, node in nodes.items():
        for h, d in node.items():
            ...
            if 0 < r < b3.DIST_VALUE_4BET["raise_2.2x"]:  # a bluff 4-bet, not value
                assert d.get("fold", 0.0) >= b3.FOLD_GATE - 1e-9, \
