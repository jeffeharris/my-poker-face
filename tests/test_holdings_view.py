...
class TestNetWorthFor(unittest.TestCase):
    ...
    def test_missing_keys_default_zero(self):
        out = _net_worth_for(
            'player:p', 'p', 500,
            receivables={}, outstanding={}, staking_pnl={},
            vice={}, side_hustle={}, rake={},
        )
...
class TestStakeAggregates(_Base):
    ...
    def test_staking_pnl_closed_only_excludes_carry_and_house(self):
        ...
        _insert_stake(self.db_path, stake_id='w', staker_id='A', borrower_id='B',
                      status='settled', principal=2000)
        self._set_payout('w', 3000)
        # Defaulted: recovered only 1200 of 2000 → −800.
        _insert_stake(self.db_path, stake_id='d', staker_id='A', borrower_id='C',
                      status='defaulted', principal=2000)
        self._set_payout('d', 1200)
        # Carry (open) — excluded; its value lives in receivable.
        _insert_stake(self.db_path, stake_id='c', staker_id='A', borrower_id='D',
                      status='carry', principal=2000, carry_amount=500)
        self._set_payout('c', 1500)
        # House stake — excluded (no entity to credit).
        _insert_stake(self.db_path, stake_id='h', staker_id=None, borrower_id='E',
                      status='settled', principal=9999, staker_kind='house')
    ...
    def _set_payout(self, stake_id, payout):
        ...
        with _sq.connect(self.db_path) as conn:
            conn.execute("UPDATE stakes SET staker_payout=? WHERE stake_id=?",
                         (payout, stake_id))
...
class TestComputeHoldingsSnapshot(_Base):
    def test_seat_stack_folds_into_chips_and_net_worth(self):
        ...
        snap = compute_holdings_snapshot(
            ...
            cash_table_repo=tables, db_path=self.db_path, sandbox_id=SANDBOX,
        )
