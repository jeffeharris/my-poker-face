...
class TestCarryAgeFactor(unittest.TestCase):
    ...
    def test_saturates_at_ramp(self):
        ...
        self.assertAlmostEqual(
            _carry_age_factor(s, ANCHOR + timedelta(days=365)),
            1.0,
        )
...
class TestAffordabilityGate(unittest.TestCase):
    def test_blocks_when_headroom_below_min_payment(self):
        ...
        self.assertFalse(_affordability_gate(floor + PAYOFF_MIN_PAYMENT - 1, 1000))
...
class TestPartialPayoff:
    ...
    def test_partial_payment_keeps_status_carry(self, db_setup):
        ...
        bankroll.save_player_bankroll(
            PlayerBankrollState(
                player_id="human_partial_1",
                chips=2_000,
        ...
        with bankroll._get_connection() as conn:
            conn.execute(
                "INSERT INTO personalities (personality_id, name, config_json) " "VALUES (?, ?, ?)",
                (
                    "borrower",
                    "Borrower",
                    json.dumps(
