...
def test_global_fill_seats_idle_grinder_at_the_fishier_table(db_path):
    ...
    fishy = CashTableState(
        table_id="cash-fishy", stake_label="$2", seats=fishy_seats,
        name="Fishy", table_type="casino",
    )
    dead = CashTableState(
        table_id="cash-dead", stake_label="$2",
        seats=[open_slot() for _ in range(6)], name="Dead", table_type="casino",
    )
...
def test_global_fill_refuses_to_seat_unfundable_ai(db_path):
    ...
    _insert_personality(
        db_path, "broke_b", name="Broke",
        knobs={"starting_bankroll": 50, "bankroll_rate": 0,
    ...
    table = CashTableState(
        table_id="cash-t", stake_label="$2",
        seats=[open_slot() for _ in range(6)], name="T", table_type="casino",
    )
    cash_table_repo.save_table(table, sandbox_id=SB)
    entry = IdlePoolEntry(
        personality_id="broke_b", left_at=now - timedelta(hours=6),
