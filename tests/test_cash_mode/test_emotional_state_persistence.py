...
class TestEmotionalStateRoundTrip:
    ...
    def test_save_creates_bankroll_row_if_missing(self, repo):
        ...
        repo.save_emotional_state_json(
            "brand_new_pid", '{"state": "confident"}', sandbox_id="test-sandbox-1"
        )
    ...
    def test_save_does_not_clobber_existing_chips(self, repo):
        ...
        repo.save_emotional_state_json(
            "napoleon", '{"state": "tilted"}', sandbox_id="test-sandbox-1"
        )
        ...
        assert (
            repo.load_emotional_state_json("napoleon", sandbox_id="test-sandbox-1")
            == '{"state": "tilted"}'
        )
