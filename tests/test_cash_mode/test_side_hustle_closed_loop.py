...
go off-grid to a side hustle (start pass), then return with a pool-funded
payout (expiry pass). Asserts the load-bearing invariants of
CASH_MODE_SIDE_HUSTLE.md:
...
def test_broke_ais_hustle_and_recover_from_pool(repos):
    ...
    # --- Tick 1: start pass sends broke AIs off-grid (capped per refresh). ---
    ...
    # No payout yet — chips move only at expiry.
    for pid in active:
        assert bankroll.load_ai_bankroll(pid, sandbox_id=SBX).chips == 10
    assert _audit_drift(repos_dict, db_path) == 0

    # --- Tick 2 (5h later): the started hustles expire and pay out. ---
