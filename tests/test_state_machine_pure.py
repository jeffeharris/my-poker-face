...
class TestPureStateMachine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.initial_state = ImmutableStateMachine(
            game_state=self.game_state, phase=PokerPhase.INITIALIZING_GAME
        )
    ...
    def test_get_next_phase(self):
        ...
        self.assertEqual(get_next_phase(self.initial_state), PokerPhase.INITIALIZING_HAND)

        # Test PRE_FLOP -> DEALING_CARDS
        pre_flop_state = self.initial_state.with_phase(PokerPhase.PRE_FLOP)
        self.assertEqual(get_next_phase(pre_flop_state), PokerPhase.DEALING_CARDS)
