...
def assign_user_group(user_id: str):
    ...
    try:
        ...
        if success:
            ...
            return jsonify({'success': True, 'message': f'User assigned to {group_name} group'})
        else:
...
def remove_user_group(user_id: str, group_name: str):
    ...
    try:
        ...
        if user_id == current_user_id and group_name == 'admin':
            return jsonify(
                {'success': False, 'error': 'You cannot remove yourself from the admin group'}
            ), 400
        ...
        if success:
            ...
            return jsonify({'success': True, 'message': f'User removed from {group_name} group'})
        else:
            return jsonify({'success': False, 'error': 'User was not in that group'}), 404
