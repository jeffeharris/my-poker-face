...
def _run_hand_instrumented(
    ...
    while sm.phase not in TERMINAL_PHASES:
        ...
        if current_player.name == hero_name:
            ...
            if action == 'fold' and hero_folded_at is None:
                ...
                community_at_fold = [card_to_string(c) for c in gs.community_cards]
                ...
                if sm.phase in (
                    ...
                    try:
                        ...
                        community = [card_to_string(c) for c in gs.community_cards]
                        node = build_postflop_node(
                            gs,
                            next(i for i, p in enumerate(gs.players) if p.name == hero_name),
                            hole,
                            community,
                        )
                    ...
                    hero_fold_required_equity = float(snap.get('required_equity') or 0.0)
        ...
        if (
            hero_controller is not None
            and phase_name == 'PRE_FLOP'
        ...
        if (
            hero_controller is not None
            and getattr(hero_controller, 'opponent_model_manager', None) is None
            and opponent_manager is not None
        ):
    ...
    reached_showdown = len(final_gs.community_cards) >= 5 and hero_folded_at is None

    hero_player = next(p for p in final_gs.players if p.name == hero_name)
    hero_hand = tuple(
        c if isinstance(c, str) else f"{c.rank}{c.suit}" for c in (hero_player.hand or ())
    )
...
def run_breakdown(
    ...
    config_hero = apply_adaptation_bias_override(ARCHETYPES[hero_archetype], hero_adaptation_bias)
    ...
    for hand_num in tqdm(range(n_hands), desc=f"  seed={seed}", leave=False, file=sys.stderr):
        ...
        controllers = [
            ...
            make_controller(
                villain_name, config_villain, strategy_table, sm, rng_seed=hand_seed + 1_000_000
            ),
        ]
        controllers[0].opponent_model_manager = opponent_manager
        opponent_manager.record_hand_dealt(
            observer=hero_name,
            opponents=[villain_name],
        ...
        if folded_at in (
            ...
            archetype = result.get('hero_fold_opponent_archetype') or 'unknown'
    ...
    return (
        ...
        fold_class_deltas,
        captured_hands,
        fold_multi_axis_counts,
        fold_multi_axis_deltas,
        fold_archetype_counts,
        fold_archetype_deltas,
        decision_counts,
        aggressive_counts,
    )
...
def print_captured_hands(hands: List[Dict], hero_name: str, villain_name: str):
    ...
    for i, hand in enumerate(hands, 1):
        ...
        print(
            f"  Hero needs to call: {hand['call_amount_at_fold']}  "
...
def print_fold_class_breakdown(
    ...
    fold_class_deltas: Dict[Tuple[str, str], int],
    n_hands: int,
    big_blind: int,
):
    ...
    print(
        f"  {'phase':<8}  {'hand_class':<16}  {'count':>6}  "
        f"{'mean Δ':>10}  {'total Δ':>12}  {'bb/100':>10}"
    )
    print(f"  {'-' * 8}  {'-' * 16}  {'-' * 6}  {'-' * 10}  " f"{'-' * 12}  {'-' * 10}")
    rows = sorted(
        fold_class_counts.items(),
        key=lambda kv: (
            ['flop', 'turn', 'river'].index(kv[0][0])
            if kv[0][0] in ('flop', 'turn', 'river')
            else 9,
            -kv[1],
        ),
    )
...
def print_value_and_bluff_freq_breakdown(
    ...
    def _print_section(title: str, classes: frozenset):
        rows = [
            ((hc, arche), count) for (hc, arche), count in decision_counts.items() if hc in classes
        ]
        if not rows:
            return
        ...
        for (hc, arche), total in rows:
            ...
            print(f"  {hc:<16}  {arche:<18}  {total:>10}  " f"{agg:>11}  {rate:>6.1f}%")
...
def print_breakdown(
    ...
    print(
        f"  {'bucket':<20}  {'count':>6}  {'pct':>6}  "
        f"{'mean Δ':>10}  {'total Δ':>12}  {'bb/100 contrib':>14}"
    )
    print(f"  {'-' * 20}  {'-' * 6}  {'-' * 6}  {'-' * 10}  " f"{'-' * 12}  {'-' * 14}")
    ...
    if fold_class_counts:
        print_fold_class_breakdown(
            fold_class_counts,
            fold_class_deltas or defaultdict(int),
            n_hands,
            big_blind,
        )
...
def main():
    ...
    if overall_multi_axis_counts:
        print_multi_axis_breakdown(
            overall_multi_axis_counts,
            overall_multi_axis_deltas,
            args.hands * len(seeds),
            100,
        )
    if overall_archetype_counts:
        print_archetype_breakdown(
            overall_archetype_counts,
            overall_archetype_deltas,
            args.hands * len(seeds),
            100,
        )
