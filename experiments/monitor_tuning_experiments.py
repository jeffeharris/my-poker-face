...
from experiments.analysis.zone_metrics_analyzer import PRD_TARGETS, ZoneMetricsAnalyzer
...
def generate_comparison_report(db_path, experiment_ids):
    ...
    for r in results:
        ...
        print(
            f"{name:<35} {fmt('baseline', tilt.baseline):>10} {fmt('medium', tilt.medium):>10} "
...
def main():
    ...
    completed_ids = [
        eid
        for eid, info in check_experiment_status(conn, experiment_ids).items()
