...
refinements rather than the safety net. Residual (optional): PRH-19 (skip the
recovery LLM call on a transport `status=="error"`), PRH-21/22 structural
(don't hold the lock across the LLM call / cancel an in-flight decision on
leave), PRH-23/24. Two FAST-tier paths still on the 600s default and *not* yet
bounded: post-hand `commentary_generator` and `leave_narrative` (the latter
already runs off-thread via a `ThreadPoolExecutor`, so it can't block the lock)
— low urgency, same one-line `default_timeout` when wanted.
