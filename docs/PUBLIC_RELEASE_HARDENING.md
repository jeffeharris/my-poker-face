...
**PRH-27 is now fully closed** — the last moderation gap (AI-personality
`avatar_description`/`visual_identity` image inputs) is screened on all three
write paths (create/update personality, update-avatar-description). Still open:
optional prompt-delimiting (defense-in-depth) + the structural PRH-33 items, the
fourth-wave browser/deploy hardening (PRH-36…41), and the Tier-1/2 items
(backups, single-worker CPU ceiling, client-side PRH-12, table-growth retention).
...
|----|---------|----------|-----------------|
| **PRH-18** ✅ | **[LANDED]** `LLMClient` now takes a `default_timeout` (and `complete(timeout=…)`) threaded to `provider.complete(timeout=…)` → the SDK `create(timeout=…)`; passed only when set, so batch/experiment callers keep the 600s shared-client default. A new `INGAME_LLM_TIMEOUT_SECONDS` (env `LLM_INGAME_TIMEOUT`, default **30s**) is wired into the in-game decision (`Assistant` in `poker_player.py`), the sharp-bot narration (`tiered_factory.py` + the expression cleanup client), and the ticker vice/side-hustle narration — so a stalled provider fails fast into the deterministic fallback instead of hanging a hand (≈30s now, not ~30 min). Honored in all OpenAI-compatible providers + Anthropic; accepted (ignored) elsewhere. Tests: `tests/test_core/llm/test_timeout.py`. Original finding below. — A silent provider stall hangs a single hand for ~30 min (≈60 min for full-LLM bots). The AI decision is synchronous inside `progress_game` under the per-game lock; on a stall it eats the 600s×3 budget. For full-LLM bots (`chaos`/`standard`/`lean`) it then runs a **second** full LLM call (recovery, see PRH-19) → another 30 min. The default `sharp` bot's *decision* is LLM-free, but its Layer-3 narration (`expression_generator.complete()`, `expression_generator.py:144`) carries the same 600s exposure (catch-all → `_empty()`, but only *after* the timeout). The existing `FallbackActionSelector` fallback is real but only fires once the budget is exhausted, so it bounds-but-does-not-prevent the hang. | `core/llm/providers/http_client.py:13,23`; `core/llm/client.py:175`; decision call `poker/controllers.py:1285`; lock `flask_app/handlers/game_handler.py:3269` | Add a **short, env-configurable per-call timeout for in-game calls** (decision + narration, e.g. 15–30s) distinct from the 600s default appropriate for batch/experiment work; thread it through `LLMClient.complete(timeout=…)` → the provider call. Optionally cap total decision wall-clock. |
| **PRH-19** ✅ | **[LANDED]** `_get_ai_decision` now inspects `llm_response.status` first: a transport-level failure (`status=="error"` — timeout / connection / budget block) routes **straight to the deterministic fallback** and skips the recovery branch, so it no longer fires a second `chat_full` against the same down provider (which doubled the hang). The capture is marked with a transport-specific description; recovery is reserved for genuine malformed-JSON / semantic errors from a live provider. Test asserts exactly one `chat_full` call + fallback used. Original finding below. — `status=="error"` was not distinguished from malformed JSON, so a transport failure triggered a *second* full LLM call. On timeout `complete()` returns `content="" status="error"` (`client.py:299-314`); the decision caller never inspects `status`/`error_code` — empty content raises `AIResponseError`, which routes into the recovery branch that makes **another** `chat_full` call against the same down provider (`controllers.py:1296-1311` → `:1358`). Doubles the hang and wastes a paid call. (This is the unaddressed M3-3 "every caller must check `status`" item, made concrete.) | `poker/controllers.py:1296-1311,1358`; `core/llm/client.py:299-314` | Check `llm_response.status`/`error_code` first; on a transport error go **straight to fallback** (skip the recovery LLM call). Reserve recovery for genuine malformed-JSON-from-a-live-provider. |
...
| ID | Finding | Location | Recommended fix |
|----|---------|----------|-----------------|
| **PRH-21** ✅ | **[RESOLVED — re-verified 2026-05-29]** Two things changed since the audit: (1) the ticker **no longer holds the per-sandbox lock** across `refresh_unseated_tables` (a prior refactor — `get_sandbox_lock` is now only taken by the sit/leave/top-up/rebuy routes), so a stalled narration can't freeze human seat ops; and (2) the vice/side-hustle narration now runs under a **dedicated 10s** timeout (`TICKER_LLM_TIMEOUT_SECONDS`, env `LLM_TICKER_TIMEOUT`) — tighter than the 30s in-game bound, because a ticker stall has an all-users blast radius. The only residual is that a stalled narration pauses the shared ticker loop for ≤10s (was ~30 min); fully off-threading it (deferred-result pattern) is **deferred as high-risk-low-reward** — `narrate_vice` returns both the flavor text *and* the duration bucket that drives the vice state/chip-move, so deferring it would re-plumb a conservation-sensitive path. Original finding below. — One stalled vice/hustle narration freezes the *entire* world ticker for *all* users (~30 min). The ticker is a single shared background loop; `_tick_sandbox` holds the per-sandbox lock across `refresh_unseated_tables`, which fires **synchronous** `narrate_vice`/`narrate_side_hustle` LLM calls ("Each fire is a sync narration call", `cash_mode/lobby.py:1845`). A stall blocks that one greenlet → no other sandbox advances, and that sandbox's human seat ops block the whole time. `CYCLE_BUDGET_MS=250` is checked only *between* sandboxes (`ticker_service.py:171`) so it cannot interrupt a blocking I/O call. Low probability per tick (narration is probability/duration gated, ≤2/refresh, FAST-tier) but **unbounded blast radius** when it hits. | `ticker_service.py:218`; `cash_mode/lobby.py:1845,1872,1951`; `cash_mode/vice_narration.py:119` | Don't make LLM calls while holding the sandbox lock — the lock only needs to guard the seat read-modify-write. Generate narration **outside** the lock, or off-thread via the existing `ThreadPoolExecutor` (`cash_mode/leave_narrative.py:327` already does this for leave narration). PRH-18's per-call timeout caps the worst case regardless. |
| **PRH-22** ✅ | **[MITIGATED — re-verified 2026-05-29]** As-good-as-feasible: `progress_game` checks `leave_requested` **before each AI decision** (cooperative cancel, `game_handler.py:3428`) and PRH-18 bounds each decision to ≤30s — so after a user hits Leave, the route gets the game lock within one (≤30s, was ~30 min) decision. True mid-call interruption isn't feasible (you can't abort a blocking sync httpx read from another thread), so PRH-18's per-call timeout *is* the cancellation bound; no clean further win. Original finding below. — A stalled AI turn blocks the human's *leave/top-up/rebuy*, not just the hand. `progress_game` holds the per-game lock across the AI decision/narration; top-up (`:2135`), **leave** (`:3853`), and rebuy (`:4444`) acquire that same lock with a **blocking** `with lock:`. So when the table looks frozen and the user hits Leave — the natural escape — that request also hangs for the full timeout window. Worse, leave then itself runs `refresh_unseated_tables` (more sync narration) under both locks. The existing `leave_requested` cooperative-cancel only checks *between* AI orbits (`game_handler.py:3290`), not mid-LLM-call. | `flask_app/handlers/game_handler.py:3269-3270,3290`; `cash_routes.py:2135,3853,4444` | PRH-18 (per-call timeout) is the primary mitigation. Optionally let leave abort an in-flight decision (cancellation token checked around the LLM call), so "get me out" is always fast. |
| **PRH-23** ✅ | **[LANDED]** `_cleanup_stale_games` now mutates `game_locks` **under `_game_locks_lock`** and **skips any game whose lock is currently held** (`lock.locked()`) — so it can't pop a lock out from under an in-flight request (which would let the next `get_game_lock` mint a 2nd lock → two concurrent progressions). Staleness is re-checked inside the lock so a game touched between snapshot and pop isn't evicted; a skipped game is collected next cycle (it's persisted, so an over-eager eviction would only force a cold-load). +2 tests. Original finding below. — TTL cleanup mutated the global game dicts without `_game_locks_lock` and could pop a lock an in-flight request holds. `_cleanup_stale_games` pops `games`/`game_locks`/`game_last_access` (`game_state_service.py:38-47`) unguarded; `get_game_lock` creates under `_game_locks_lock` (`:150-153`). If a game were evicted while a request held its lock, the next request would mint a **new** lock for the same id → two concurrent progressions. Very low likelihood (2h-idle games aren't being progressed; stale list is built before popping, so no iterate-while-mutate), but a latent correctness gap. | `flask_app/services/game_state_service.py:38-47,150-153` | Take `_game_locks_lock` in cleanup; skip eviction when `lock.locked()`. |
| **PRH-24** ✅ | **[LANDED — env-configurable + runtime-confirmed + documented]** `async_mode` is now env-driven via `SOCKETIO_ASYNC_MODE` (default `threading`, unchanged), so prod can adopt the standards-aligned `gevent` pairing after validating the WebSocket flow without a code change. A startup runtime check (`_log_async_runtime`) logs `[ASYNC] async_mode=… monkey-patch active=…` — turning the "does it actually yield cooperatively?" assumption into an observable fact — and **escalates the genuinely-broken case** (prod + threading + NOT monkey-patched) to an alertable ERROR (pages via PRH-28). Documented alongside the Redis/`-w 1` constraint in `SECURITY_POSTURE.md`. Per the audit's own correction, the gevent worker *does* monkey-patch, so the current pairing is safe — this makes it explicit + switchable rather than blind-flipping a live socket layer. | `flask_app/config.py` `SOCKETIO_ASYNC_MODE`; `flask_app/extensions.py`; `flask_app/__init__.py` `_log_async_runtime` | Done. |
...
**✅ PRH-18 landed (2026-05-29)** — `LLM_INGAME_TIMEOUT` (default 30s) is wired
into the in-game decision + sharp-bot/ticker narration clients (see the PRH-18
row). This bounds the worst case for the decision hang **and** the ticker
freeze (PRH-21) and the leave/top-up/rebuy block (PRH-22), so those are now
refinements rather than the safety net. **Update 2026-05-29:** ~~PRH-19~~ ✅
(transport-error short-circuit), ~~PRH-21~~ ✅ (ticker lock already removed by a
prior refactor + dedicated 10s ticker-narration timeout), ~~PRH-22~~ ✅
(cooperative-cancel + PRH-18 bound; mid-call interruption not feasible) all
closed/mitigated. **Still open:** PRH-23 (TTL-cleanup lock — latent), PRH-24
(standardize the async model). Two FAST-tier paths still on the 600s default and
*not* yet bounded: post-hand `commentary_generator` and `leave_narrative` (the
latter already runs off-thread via a `ThreadPoolExecutor`, so it can't block the
lock) — low urgency, same one-line `default_timeout` when wanted.
...
| **PRH-27** ✅ | **No content moderation on an unmoderated UGC+LLM pipeline.** In-game chat is appended verbatim into the AI's decision prompt (prompt-injection / offensive-output / cost surface); personality/theme *names* are interpolated into generation prompts; owner-flippable **public** personalities expose user names + LLM bios to all users. **PARTIALLY MITIGATED (guest chat policy landed):** guest **free-text** chat is sign-in-gated (`GUEST_FREE_CHAT_ENABLED`, default off; `check_guest_free_chat` on both the HTTP and socket send paths); guests keep **quick-chat** (bounded tone vocabulary) **once per turn** (the per-turn cap is `check_guest_message_limit`); client locks the keyboard tab and defaults guests to quick-chat. **Residual:** a scripted guest could attach a valid `tone` to arbitrary text (bounded to 1/turn + the budget cap) — closed only by the moderation work below. **Public personality sharing is now admin-only (landed):** the visibility endpoint requires admin to set `public`/`disabled` (non-admin owners may only set `private`); the UI hides the publish option for non-admins; and `save_personality` now **preserves** an existing row's `visibility`/`owner_id` on re-save — fixing a latent bug where editing an avatar/visual-identity (`generator.set_avatar_description`/`set_reference_image_id`, `character_images` retry) silently published + orphaned a private personality. **Profile bio + avatar prompt moderation (landed):** after merging `development` (which adds the human player profile — `user_preferences.bio`, AI-visible + table-visible — and avatar generation), `core/moderation.py` (`moderate_text`, OpenAI omni-moderation, free) now screens the bio (`set_bio` route) and the avatar text prompt (`generate_avatar` / `generate-photo`); flagged content → 400 `MODERATION_REJECTED`. Fail-closed on a positive hit, fail-open on outage, no-op without `OPENAI_API_KEY` or with `MODERATION_ENABLED=false`. The bio is already length-capped server-side (`set_bio` 500 chars). **Authed in-game chat + name moderation (landed):** the player-chat send paths (HTTP `api_send_message` + socket `handle_send_message`) now length-cap (`MAX_PLAYER_CHAT_LEN=500`, `CHAT_TOO_LONG`) and `moderate_text` free-text before it reaches the AI prompt → flagged `MODERATION_REJECTED`; personality/theme name + description generation (`create_personality`, `generate_personality`, `generate_theme`) moderate their user text too. **AI-personality image inputs (landed — the last gap):** `avatar_description` and the `visual_identity` subfields (identity/appearance/apparel) are interpolated into the paid image-generation prompt (`poker/image_prompt_config.py`); they are now screened on every write path — `create_personality` (name + image text), `update_personality`, and `update_avatar_description` — via `_personality_image_text` + `_moderation_error`. The regenerate endpoint reads the *stored* description (`image_routes.py:456`), so write-path screening covers the generation reads. **PRH-27 is now FULLY closed** — guest free-chat lock, admin-only sharing, profile bio/avatar moderation, authed chat/name moderation, and personality image-input moderation all landed. Remaining (low sev, optional): explicit prompt-delimiting of user content (defense-in-depth on top of moderation). *(The structural sender-spoof / unbounded-length items, PRH-33, are now closed.)* | chat→prompt `message_handler.py:123`→`controllers.py:559`; **chat moderation** `game_routes.py` `_player_chat_rejection`; **name + image-input moderation** `personality_routes.py` `_moderation_error`/`_personality_image_text`; **publish gate** visibility route + `save_personality` (preserve); **bio/avatar moderation** `core/moderation.py` + `profile_routes.py` | Remaining (optional): prompt-delimiting; PRH-33 (force `sender` server-side). |
...
| ID | Finding | Location | Recommended fix |
|----|---------|----------|-----------------|
| **PRH-29** ◑ | **[LANDED — script + deploy wired; cron/off-box are operator steps]** New `scripts/backup_db.py` uses the SQLite **online backup API** (consistent snapshot of a live WAL DB), runs `PRAGMA integrity_check` (deletes + non-zero-exits on corruption — alertable), prunes to `--keep` daily (default 7), and ships off-box via `--remote-cmd`/`BACKUP_REMOTE_CMD` (`{path}` substituted; rclone/rsync/aws). `deploy.sh` now calls it (WAL-safe, keep 7) instead of the torn `cp`; a failed/corrupt backup aborts the deploy. Verified end-to-end (snapshot of an uncheckpointed-WAL DB, integrity ok, retention, off-box success + failure exit code). **Operator steps (can't be done from the repo):** add the documented **cron** (daily, off-box `--remote-cmd`) and provision the remote (Storage Box/S3). | `scripts/backup_db.py`; `deploy.sh` | Done in-repo; schedule the cron + set `BACKUP_REMOTE_CMD` on the box. |
| **PRH-30** ◑ | **[LANDED — MC lever; save-coalescing deliberately deferred]** The per-decision equity Monte Carlo (analytics, *not* the bot's actual decision) is now env-configurable via `DECISION_ANALYSIS_ITERATIONS` (read in `get_analyzer`; default 2000 so behavior is unchanged unless opted in) and **prod is set to 500** (~4× less per-decision CPU, raising the concurrent-hands ceiling; coaching/on-demand analyzers are unaffected). +3 tests. **Not done (by design):** coalescing the per-action full-game saves to hand boundaries — those per-action writes are exactly what gives cold-load **mid-hand recovery**; coalescing trades durability (a restart would lose a whole hand, not sub-second progress) for CPU, which isn't a safe unilateral change. | `poker/decision_analyzer.py` `get_analyzer`; `docker-compose.prod.yml` | MC lever done; save-coalescing left as a deliberate durability tradeoff to decide explicitly. |
| **PRH-31** ✅ | **[LANDED]** The client now completes the PRH-12 self-heal: `handlePlayerAction` detects a `409` with `code: RELOAD_REQUIRED`, calls `refreshGameState` (which rehydrates via the GET cold-load path) and **retries the action once**; and a new `reload_required` socket listener refreshes state on the server's push. So a tap right after a reconnect/eviction self-heals instead of reading as a dead button. tsc + eslint clean. Original finding: client listened for neither signal. | `react/.../usePokerGame.ts` (`handlePlayerAction` 409 retry + `reload_required` listener) | Done. |
| **PRH-32** ✅ | **[LANDED]** A daily retention sweep (`flask_app/services/retention_service.py`, started in `create_app`) now enforces both windows: `prompt_captures` via the previously-unscheduled `cleanup_old_captures(get_retention_days())`, and `api_usage` via a new `UsageTracker.prune_old_usage(API_USAGE_RETENTION_DAYS)`. Both treat 0/unset as keep-all (inert in dev/tests). **Prod defaults set:** `LLM_PROMPT_RETENTION_DAYS=30`, `API_USAGE_RETENTION_DAYS=90` (override via host env). VACUUM intentionally skipped (write-lock stall risk on the single worker; SQLite reclaims lazily). +3 tests. | `flask_app/services/retention_service.py`; `core/llm/tracking.py` `prune_old_usage`; `docker-compose.prod.yml` | Done. |

### Tier 2 — medium / polish

| ID | Finding | Location | Recommended fix |
|----|---------|----------|-----------------|
| **PRH-33** ✅ | **[LANDED]** Both send paths (HTTP `api_send_message` + socket `handle_send_message`) now force `sender` to the **human's actual seat name** via `_human_seat_name(game_data)` (reads the `is_human` player off the live game state), so a spoofed `sender` can't be injected into the AI prompt or the relationship-event attribution. The server-side length cap (`MAX_PLAYER_CHAT_LEN=500`, `CHAT_TOO_LONG`) landed earlier with PRH-27. +4 tests (helper + tolerance). | `game_routes.py` `_human_seat_name` + both send paths | Done. |
| **PRH-34** ✅ | **[LANDED]** `BaseRepository` instances now register in a module `WeakSet`; `close_all_thread_connections()` closes the current thread/greenlet's connection on every live repo, and a `teardown_appcontext` hook in `create_app` calls it at the end of each request **and** socket event (flask-socketio pushes an app context per event). Connections are still reused across all DB ops *within* a request/event — they're released at the end instead of accumulating WAL readers + fds over uptime under the greenlet-recycling worker. Tests: close releases + reopens lazily; teardown-hook integration. Full `-m flask` sweep green. | `poker/repositories/base_repository.py`; `flask_app/__init__.py` teardown hook | Done. (Also reduces — but doesn't fully eliminate — the single-process test-suite fd pressure, since many tests use repos outside an app context; the `ulimit -n` + xdist run remains the suite recipe.) |
| **PRH-35** ◑ | **[LANDED — structured logs + request IDs; log-shipping is ops]** `flask_app/logging_setup.py` replaces the bare `basicConfig`: a `LogRecordFactory` stamps `request_id` on **every** record (so the stdout handler *and* the PRH-28 alert webhook carry it — an alert now shows `req=<id>`), an optional JSON formatter (`LOG_FORMAT=json`) emits machine-parseable lines for an aggregator, and `init_request_logging` gives each HTTP request a correlation id (honors an inbound `X-Request-ID`, else mints one) echoed in the `X-Request-ID` response header. `LOG_LEVEL` controls the threshold. +5 tests. **Residual:** shipping stdout to an actual aggregator + Socket.IO-event ids are ops/follow-ups (Socket.IO events don't hit the HTTP hooks, so they log `request_id=-`). | `flask_app/logging_setup.py`; `flask_app/__init__.py`; `alerting.py` (`req=` tag) | Done in-app; set `LOG_FORMAT=json` + wire stdout to an aggregator in prod. |
...
**Tier-0 status:** ~~PRH-25 (arm the budget)~~ ✅ → ~~PRH-28 (alerting sink)~~ ✅ (set `ALERT_WEBHOOK_URL` to activate) → ~~PRH-26 (guest-minting throttle + bot/auth gating)~~ ✅ → ~~PRH-27 (content moderation — guest chat, authed chat/names, profile bio/avatar, AND AI-personality image inputs)~~ ✅. **All Tier-0 blockers closed; PRH-27 fully closed.** Remaining go-public polish: fourth-wave browser/deploy hardening (PRH-36…41), then Tier 1 (backups PRH-29, single-worker CPU ceiling PRH-30, client-side PRH-12 self-heal PRH-31, table-growth retention PRH-32).
...
| ID | Finding | Location | Recommended fix |
|----|---------|----------|-----------------|
| **PRH-36** ✅ | **[LANDED — armed in prod]** Double-submit-cookie CSRF. `flask_app/csrf.py` issues a non-HttpOnly `csrf_token` cookie (`after_request`, set when absent) and requires `X-CSRF-Token` to equal it on every mutating (`POST`/`PUT`/`PATCH`/`DELETE`) `/api/*` request (`before_request`, constant-time compare → `403 CSRF_FAILED`). The frontend attaches the header via **one global `fetch` wrapper** (`react/.../utils/csrf.ts`, installed in `main.tsx`) scoped to our own API — so all ~109 mutating call sites are covered without per-site plumbing. Enforcement is gated by `CSRF_PROTECTION_ENABLED`, **default ON in production** (SPA+API are same-origin there, so the cookie is JS-readable) and **OFF in dev** (cross-origin `:5173↔:5000`, cookie not readable) **and tests**. Exempt: `OPTIONS` preflight, `/api/auth/login`, `/api/auth/google/*` (auth bootstrap / external OAuth redirect carrying its own `state`). Socket.IO is a separate transport guarded by the CORS origin allowlist. 8 route tests (reject/accept/exempt/disabled). Original finding: only `SameSite=Lax` defense-in-depth, no first-class token. | `flask_app/csrf.py`; flag `flask_app/config.py` `CSRF_PROTECTION_ENABLED`; frontend `react/react/src/utils/csrf.ts` + `main.tsx`; tests `tests/test_csrf.py` | Done. (Ops: armed by default when `FLASK_ENV=production`; the SPA must be served same-origin as the API — it is, via nginx — for the cookie to be JS-readable.) |
| **PRH-37** ✅ | **[LANDED]** Browser flows are now cookie-only. Guest login no longer returns a bearer token, and the frontend no longer reads/writes/sends an `authToken` from `localStorage` — `/api/auth/me`, `/api/usage-stats`, and the Socket.IO handshake all authenticate via the HttpOnly session + signed guest cookies (`credentials: 'include'` / `withCredentials: true`, already in place). `logout` still scrubs any legacy `authToken`. The server *retains* Bearer acceptance in `get_current_user` for non-browser/API clients (dormant — nothing issues one to a browser), so no browser-readable token exists to steal under a future XSS. Original finding: JWTs stored in `localStorage` increased XSS blast radius and were redundant given the HttpOnly cookies. | Login no-token `poker/auth.py` (guest branch); token removal `react/react/src/hooks/useAuth.tsx`, `UsageStatsProvider.tsx` | Done. (If scoped API tokens are needed later, issue them separately, short-lived, never in long-lived browser storage.) |
| **PRH-38** ✅ | **[LANDED]** In production a `guest_`-namespaced admin is refused at both seams: `initialize_admin_from_env` returns None (logs an ERROR) for a `guest_` `INITIAL_ADMIN_EMAIL`, and `assign_user_to_group('admin', …)` raises for any `guest_` id when `FLASK_ENV=production` — even the configured initial-admin. The guest-namespace exception remains a dev-only convenience. Original finding: prod admin could be tied to a forgeable/ephemeral guest namespace. | `poker/repositories/user_repository.py` (`assign_user_to_group`, `initialize_admin_from_env`); tests `tests/test_repositories/test_user_repository.py` | Done. (Break-glass recovery + step-up admin controls — IP allowlist / Cloudflare Access — remain optional follow-ups.) |
| **PRH-39** ◑ | **[LANDED — headers set; CSP script-src tightening deferred]** The frontend nginx now sets, at the server level (inherited by the proxied `/api` + `/socket.io`, `always`): `X-Frame-Options: DENY` + CSP `frame-ancestors 'none'` (clickjacking), `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy` (geolocation/mic/camera/payment/usb off), and a CSP with `default-src 'self'`, `object-src 'none'`, `base-uri 'self'`, scoped `img-src`/`connect-src` (ws/wss). `script-src`/`style-src` keep `'unsafe-inline'` because `index.html` ships inline bootstrap scripts and Vite/Tailwind inject inline styles. nginx config syntax verified (`nginx -t`). | `react/react/nginx.conf` | **Residual (follow-up):** tighten `script-src` by moving the two inline `index.html` scripts into bundled JS (or nonces/hashes) so `'unsafe-inline'` can be dropped from `script-src`. |
| **PRH-40** ✅ | **[LANDED]** The base `Dockerfile` `CMD` now defaults to **gunicorn** (the prod WSGI server), so a deploy that forgets `command:` lands on the safe server, not `flask run`. `flask_app/ui_web.py` **fails closed** — it raises rather than starting the Werkzeug dev server (`debug`/`allow_unsafe_werkzeug`) when `FLASK_ENV != development`. The container runs **non-root**: the image adds an `appuser` (uid 1000) + `gosu`; the entrypoint, when `DROP_PRIVILEGES=1` (set in `docker-compose.prod.yml`), chowns the bind-mounted `/app/data` and re-execs as `appuser`. Dev keeps `command: python -m flask_app.ui_web` + root (no flag), so the local workflow is unchanged. Verified by a real image build + boot: process runs as uid 1000, gunicorn launches, `/app/data` writable. | `Dockerfile`; `bin/docker-entrypoint.sh`; `docker-compose.prod.yml`; `flask_app/ui_web.py` | Done. |
| **PRH-41** ◑ | **[LANDED — per-user quotas + velocity telemetry; concurrent-session caps deferred]** Two pieces: **(1) per-user rate-limit key** — `get_rate_limit_key` now keys real (OAuth) accounts on their stable user id instead of IP, so every existing per-route cap (coach 5–30/min, personality/theme/image generation, game actions, chat suggestions — all already present) binds **per-user** and can't be multiplied by rotating IPs. Guests stay IP-keyed (cookie-resettable id; minting throttled per IP — PRH-26). **(2) spend-velocity telemetry** — the spend gate emits a throttled `[LLM BUDGET] velocity` WARNING (pages via the PRH-28 webhook) when a scope (global or a single owner) crosses `LLM_BUDGET_VELOCITY_WARN_FRACTION` (default 0.8) of its daily cap, so a spike is visible *before* exhaustion. +8 tests. **Deferred:** concurrent-session caps, and true short-window *rate*-spike detection (the current signal is fraction-of-daily-cap, count via the existing per-route limits). | `flask_app/extensions.py` `get_rate_limit_key`; `core/llm/budget.py` `_maybe_warn_velocity` | Per-user/per-feature quotas + velocity alerting done; concurrent-session limits remain optional. |

**Highest leverage from this wave:** ~~PRH-36 (CSRF)~~ ✅ + ~~PRH-37 (remove browser-readable bearer tokens)~~ ✅ form the browser-session hardening baseline — **both landed**; the browser session is now cookie-only with a first-class CSRF token. **The entire fourth wave is now landed:** ~~PRH-38~~ ✅ (admin-not-guest), ~~PRH-39~~ ✅ (security headers; `script-src` tightening residual), ~~PRH-40~~ ✅ (prod-safe image + non-root), ~~PRH-41~~ ◑ (per-user quotas + spend-velocity telemetry; concurrent-session caps deferred).
