...
handler), and PRH-27's guest chat policy (free text locked; quick-chat allowed
once per turn). Still open: PRH-26 (guest-minting throttle), the rest of PRH-27
(moderation on authed chat/names + public personalities), and the Tier-1/2 items.
...
**Suggested Tier-0 ordering:** ~~PRH-25 (arm the budget)~~ ✅ → ~~PRH-28 (alerting sink)~~ ✅ (set `ALERT_WEBHOOK_URL` to activate) → **PRH-26 next** (guest-creation throttle + bot/auth gating; PRH-27's guest chat policy already shipped) → the rest of PRH-27 (moderation on authed chat/names + public personalities). The financial controls are now armed and watched; PRH-26 closes the remaining anonymous-abuse vector.
