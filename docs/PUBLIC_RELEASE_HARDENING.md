...
**Highest leverage from this wave:** ~~PRH-36 (CSRF)~~ ✅ + ~~PRH-37 (remove browser-readable bearer tokens)~~ ✅ form the browser-session hardening baseline — **both landed**; the browser session is now cookie-only with a first-class CSRF token. Remaining from this wave: **PRH-41** (per-feature/per-user quotas + abuse telemetry). ~~PRH-38~~ ✅ (admin-not-guest), ~~PRH-39~~ ✅ (security headers; `script-src` tightening residual), ~~PRH-40~~ ✅ (prod-safe image + non-root) all landed.
