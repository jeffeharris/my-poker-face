...
|----|-------|----------|-------------|
| T1-01 | `from_dict` copy-paste bug | `poker/poker_action.py:50-52` | Passes `dict_data['player_action']` as both player name and action. Player name gets replaced with action string. (Dead code today — but if ever called, would corrupt state.) |
| T1-02 | Hand evaluator sort bug | `poker/hand_evaluator.py:871-873` | Calls `sorted()` on already-sorted values, breaking element-by-element comparison. Can determine wrong winner in edge cases. |
| T1-03 | Two-pair kicker calculation | `poker/hand_evaluator.py:165` | Returns single kicker value instead of proper list. Wrong winner determination in two-pair scenarios. |
| T1-04 | `_check_two_pair` includes trips | `poker/hand_evaluator.py:162` | Uses `count >= 2` which matches trips/quads. Should be `== 2` for pair detection. |
| T1-05 | Raise validation bypass | `poker/poker_game.py:488` | `validate_and_sanitize()` called but sanitized amount not enforced. AI can pass invalid raise amounts. |
| T1-06 | Max raise logic non-standard | `poker/controllers.py:728` | `max_raise = min(player_stack, max_opponent_stack, pot * 2)` — pot×2 cap is not standard poker rules. Artificially limits raises. |
| T1-07 | `player_idx=0` falsy bug | `poker/poker_game.py:394` | `player_idx = player_idx or game_state.current_player_idx` — `player_idx=0` (first player) incorrectly defaults. Should use `if player_idx is None:`. |
| T1-08 | Infinite loop risk | `poker/poker_game.py:618-645` | `get_next_active_player_idx` returns inactive player if no active players found. Can cause invalid game states. |
| T1-09 | Missing `max_winnable` data | `poker/controllers.py:806-818` | Enricher callback builds `max_winnable` but `all_players_bets` isn't available in scope. Can crash or give wrong pot odds. |
| T1-10 | Name collision in player comparison | `poker/poker_game.py:437` | `reset_player_action_flags` compares players by `name` instead of index. Fails if two players share a name. |

### UX Blockers

| ID | Issue | Location | Description |
|----|-------|----------|-------------|
| T1-11 | No React error boundaries | All React components | Single component error crashes entire app to white screen. No graceful degradation. |
| T1-12 | Socket memory leak | `react/src/hooks/useSocket.ts:19-24` | `onConnect`/`onDisconnect` listeners added but never removed in cleanup. Memory leak on remount. |
| T1-13 | Missing loading/error states | `react/src/App.tsx:136-174` | `handleQuickPlay` only shows loading on success path. Errors fail silently — user clicks, nothing happens. |
| T1-14 | No offline detection | React app | No `navigator.onLine` check or offline banner. Network drop → frozen UI with no feedback. |
| T1-15 | Silent API failures | All React fetch calls | Failed API calls logged to console only. User never informed. Need toast notification system. |

### Security Basics

| ID | Issue | Location | Description |
|----|-------|----------|-------------|
| T1-16 | Predictable game IDs | `flask_app/routes/game_routes.py:189-191` | `generate_game_id()` uses `time.time() * 1000`. Easily guessable. Use `secrets.token_urlsafe()`. |
| T1-17 | No input validation on actions | `flask_app/routes/game_routes.py:979-1024` | `/api/game/<game_id>/action` doesn't validate player exists, it's their turn, or action is valid. Negative amounts allowed. |
| T1-18 | Unprotected WebSocket handlers | `flask_app/routes/game_routes.py:1198-1286` | `on_join`, `handle_player_action` — no auth checks. Anyone can join any game and take actions. |
| T1-19 | SECRET_KEY regenerated on restart | `flask_app/config.py:18` | Falls back to `os.urandom(32).hex()` if not set. Invalidates all sessions on restart. |
| T1-20 | SQL injection risk in admin | `flask_app/routes/admin_dashboard_routes.py:1724` | Table names via f-string interpolation. Whitelist check exists but pattern is dangerous. |
...
| T2-05 | DB connection created per config lookup | `flask_app/config.py:44-94` | Config getter functions like `get_default_provider()` instantiate `GamePersistence()` on every call. New DB connection per lookup. |
...
| T2-09 | O(n²) player flag reset | `poker/poker_game.py:422-437` | Calls `update_player` per player in loop, each creating new tuple copy. Build new tuple in one pass. |
...
| T2-12 | 120 console.log statements in production | 38 React files | No logging levels. Sensitive data in browser console. Performance overhead. |
...
| T2-14 | Shuffle mutates module-level list | `poker/utils.py:86` | `random.shuffle(celebrities_list)` mutates in-place. Use `random.sample()` instead. |
| T2-15 | `setup_helper.py` references non-existent file | `setup_helper.py:123` | Says `python working_game.py` — file doesn't exist. Script is broken. |
...
| T2-17 | HTTP client never closed | `core/llm/providers/http_client.py:10-17` | Module-level `httpx.Client()` singleton has no shutdown hook. Connection leak in long-running processes. |
| T2-18 | UsageTracker singleton not thread-safe | `core/llm/tracking.py:84-110` | `_instance` check-and-set has race condition. Multiple threads can create multiple instances. |
| T2-19 | Unbounded game state memory growth | `flask_app/services/game_state_service.py:14-16` | `games` dict stores all active games. Abandoned games never evicted. No TTL or LRU. |
| T2-20 | Unbounded message list growth | `flask_app/handlers/message_handler.py:82-126` | Messages appended without limit. Long games accumulate unbounded messages in memory. |
...
| T2-22 | Conversation memory trims by count, not tokens | `core/llm/conversation.py:58-61` | Trims at 15 messages regardless of token count. Long system prompts + 15 messages can exceed context limits. |
...
| T3-04 | Skipped tests not tracked | `test_prompt_golden_path.py:192` | `@unittest.skip` with no GitHub issue. Tests forgotten. |
...
| T3-13 | Hardcoded 600s HTTP timeout | `core/llm/providers/http_client.py:16` | 10-minute timeout for all operations. Can't configure per-request. |
...
| T3-16 | DB path logic duplicated 3+ times | `core/llm/tracking.py:34-52`, `flask_app/config.py:98-101`, `scripts/dbq.py:27-31` | Each with different fallback paths. Include hardcoded absolute paths. |
...
| T3-20 | `GET` allowed on destructive endpoint | `flask_app/routes/game_routes.py:1114` | `/api/end_game/<game_id>` accepts both GET and POST. GET should never mutate. |
...
| T3-24 | No `.editorconfig` | Project root | No editor settings for tabs/spaces, line endings. |
| T3-25 | No dependabot or renovate | Missing `.github/dependabot.yml` | Dependency updates manual. Security patches could be missed. |
| T3-26 | `__pycache__` files committed | `tests/` | `.gitignore` incomplete. Merge conflicts on cache files. |
| T3-27 | Makefile uses deprecated `docker-compose` | `Makefile:10-46` | Should use `docker compose` (v2). Won't work on newer systems. |
| T3-28 | GitHub Actions allows 50 lint warnings | `.github/workflows/deploy.yml:57` | `--max-warnings=50` should be 0 for clean codebase. |
...
| Tier | Count | Focus |
|------|-------|-------|
| **Tier 1: Must-Fix** | 20 | Correctness bugs, UX blockers, security basics |
| **Tier 2: Should-Fix** | 34 | Architecture, reliability, frontend quality, DevOps |
| **Tier 3: Post-Release** | 34 | Testing, performance, code org, docs, non-urgent security |
| **Total** | 88 | |
