...
| T3-55 | Duplicated psychology pipeline between experiments and game handler | `poker/psychology_pipeline.py` | **FIXED**: Extracted unified `PsychologyPipeline` class that both `game_handler.py` and `run_ai_tournament.py` invoke. Removed ~480 lines of duplicated detection/resolution/recovery code. Unified divergences: opponent logic, session_context, key_moment, clear_hand_bluff_likelihood, and state persistence. | FIXED |
