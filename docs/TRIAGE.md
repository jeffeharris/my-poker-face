...
| T3-55 | Duplicated psychology pipeline between experiments and game handler | `experiments/run_ai_tournament.py`, `flask_app/handlers/game_handler.py` | The post-hand psychology pipeline (event detection → resolve → persist → recover → persist recovery → save state) is implemented separately in both codepaths. Every feature added to one side must be manually replicated on the other, causing recurring telemetry gaps. **Known divergences**: (1) equity shock detection missing in experiments, (2) nemesis events missing in experiments, (3) recovery rate differs (0.15 vs 0.1), (4) zone gravity persistence missing in games, (5) emotional state persistence missing in games, (6) `on_hand_complete()` not called in experiments. **Fix**: Extract shared `PsychologyPipeline` class in `poker/` that both callers invoke with their game context — owns the full detect → resolve → persist → recover → save cycle. Related to T3-45 and T3-47 (both files are oversized and would benefit from this extraction). | |
