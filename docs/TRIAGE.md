...
| T1-38 | Frontend: `PipelineTracePanel` crashes on undefined trace fields | `react/react/src/components/admin/DecisionAnalyzer/PipelineTracePanel.tsx:108, 120, 126, 132` | `trace.effect_size.toFixed()` and `Object.keys(trace.inputs/config_snapshot/extra)` crash if those fields are absent (legacy DB rows or schema mismatch). Types declare them non-nullable but backend can send partial data. | |
| T1-39 | Experiment chat tests fail "Authentication required" | `tests/test_chat_persistence.py`, `tests/test_experiment_routes.py` | Five tests fail with 500 errors because T1-24's admin gate on `experiment_routes` blueprint isn't mocked in these tests. Either update test fixtures to patch `authorization_service` or refactor the gate. | |
...
| T2-36 | Chat-suggestion endpoints missing ownership check | `flask_app/routes/stats_routes.py:322, 428, 612` | `get_chat_suggestions`, `get_targeted_chat_suggestions`, `get_post_round_chat_suggestions` check existence but not ownership. Authenticated user with any `game_id` can trigger LLM calls billed to the server and read another user's game state (cards, pot, messages). | |
| T2-37 | `generate-character-images` is unauthenticated | `flask_app/routes/image_routes.py:494` | Only a rate limiter — no auth check. Anonymous callers can trigger image-generation API calls for any personality. | |
| T2-38 | Codex: strategy mapper uses big_blind for min-raise instead of last raise size | `poker/strategy/action_mapper.py:49` | `min_raise = highest_bet + big_blind`, but poker rules require `highest_bet + min_raise_amount` (size of prior raise). In re-raise spots the tiered bot chooses an "illegal" size that the engine sanitizes upward — bot commits more than the sampled strategy intended. Same issue in the postflop mapper. | |
| T2-39 | `_EXPLOITATION_RULE_ORDER` missing `bluff_reduction` entry | `poker/tiered_bot_controller.py:117-125` | Comment says it mirrors `compute_exploitation_offsets_with_traces` (8 entries) but has only 7. Early-out path emits 7 no-op traces; hot path emits 8. Breaks attribution-analysis invariant ("every rule emits a trace every decision"). | |
| T2-40 | Defense floor `hand_class == 'air'` check is dead code | `poker/strategy/defense_floor.py:117`, `poker/tiered_bot_controller.py:862-863` | Controller passes `simplify_hand_class()` output which returns `'air_no_draw'`/`'air_strong_draw'`, never literal `'air'`. Air-hand exclusion only saved by `nut_status == NUT_BLUFF_CATCHER` secondary check. Fragile to future nut_status logic changes. | |
| T2-41 | `_apply_math_floor` uses `big_blind = 0` fallback | `poker/tiered_bot_controller.py:2278` | Uses `getattr(game_state, 'current_ante', 0) or 0` instead of `big_blind_of(game_state)` (which falls back to 50). When `current_ante` is missing, `stack_bb = inf` and short-stack rule never fires. Inconsistent with `_build_decision_context` in same class. | |
| T2-42 | Zone gravity documented as implemented but never executes | `poker/psychology_pipeline.py:476-489`, `poker/player_psychology.py:651-658` | `_apply_recovery()` checks `recovery_info.get('gravity_conf'/'gravity_comp')` but `recover()` never returns those keys. `GRAVITY_STRENGTH` config never read. `PSYCHOLOGY_ZONES_MODEL.md` marks zone gravity as ✅ implemented. | |
...
| T2-44 | `record_hand_dealt(hand_number=None)` bypasses idempotency guard | `poker/memory/opponent_model.py:800-813` | Guard `if hand_number is not None and hand_number == _last_hand_dealt: return` is skipped entirely when `hand_number=None`. Any caller passing `None` twice double-increments `hands_dealt`. | |
| T2-45 | `DecisionAnalyzer.calculate_equity_vs_random` uses global `random.shuffle` | `poker/decision_analyzer.py:498` | `random.shuffle(deck)` mutates global state. Sibling method `calculate_equity_vs_ranges` correctly uses `rng = random.Random()`. Inconsistent and violates CLAUDE.md FP rule. | |
| T2-46 | `get_play_style_label` uses `hands_observed` while VPIP uses `hands_dealt` | `poker/memory/opponent_model.py:578` | After the opportunity-normalized rework, `hands_dealt >= hands_observed`. Tight-passive player with 30 dealt / 10 observed gets correct VPIP but style label fires on 10-sample pool. Mismatch between numerator/denominator. | |
| T2-47 | `pot_contributions` accumulates raise-TO and call-cost in same unit | `flask_app/routes/stats_routes.py:137` | `RecordedAction.amount` is raise-TO for raise (target total) and call-cost for call. Accumulating `action.amount` and picking `max(...)` for opponent selection misranks players in multi-way pots with mixed action types. Wrong opponent named in post-round chat. | |
| T2-48 | `board_analyzer._extract_rank` corrupts `10`-rank cards | `poker/board_analyzer.py:21-23` | `return card[0]` only safe for `card_to_string()`-normalized single-char rank. If called with `'10h'` returns `'1'`, causing `_rank_index` to raise `ValueError`. Safe today via `card_to_string()` upstream but fragile. | |
| T2-49 | Historical opponent block renders for `total_hands=0` | `flask_app/services/coach_engine.py:471` | `_get_opponent_stats` populates the `historical` block whenever the name is in historical_data, regardless of `total_hands`. Coach prompt can show `VPIP/PFR/AF: 0%/0%/0.0` for stale empty records. | |
| T2-50 | `addressing` list parsed when `should_speak=False` | `poker/memory/commentary_generator.py:515-520` | YAML instructs LLM to speak only if `chattiness > 0.4`, but `addressing` is parsed regardless of `should_speak`. Ghost callouts can land in `HandCommentary.addressing` while `table_comment` is `None`. | |
| T2-51 | `composure_state` big-loss threshold hardcoded $1000 | `poker/psychology_model.py:201` | `amount < -1000` ignores blind size. `big_blind` parameter passed in but unused. In 10BB game $1000=100BB (tournament-ending); in 100BB game $1000=20BB (one hand). Should be `amount < -(10 * big_blind)`. | |
| T2-52 | Severely-tilted player's narrative suppressed | `poker/player_psychology.py:918` | `get_prompt_section()` returns `""` if `is_severely_tilted`. Zone effects still apply but personality-authentic narrative disappears at exactly the moment it's most expressive. Verify intent. | |
| T2-53 | Coach `effective_stack`/`SPR` regress `test_owner_can_act_on_human_turn` | `flask_app/routes/game_routes.py:1591`, `flask_app/services/coach_engine.py:637`, `tests/test_websocket_auth.py:236` | Two cascading issues: (1) `handle_player_action` computes `max(0, min(pre_action_state.highest_bet - current_player.bet, current_player.stack))` at `game_routes.py:1591` — fails with `'<' not supported between MagicMock and int` because the test's `pre_action_state` MagicMock has no numeric attrs. (2) `compute_coaching_data` then errors with `player, player_idx = player_info` unpacking 0 values at `coach_engine.py:637`. Test mocks need real numeric `highest_bet`/`bet`/`stack` and a non-empty `player_info` tuple. Re-confirmed failing after the dev → hybrid-ai merge on 2026-05-15. | |
| T2-54 | Personality determinism regression | `tests/test_personality_responses.py:244` | Same scenario now returns `'call'` instead of `'fold'`. Likely caused by psychology refactor / bounded options / strategy module change. Could be intentional but warrants investigation. | |
| T2-55 | Prompt preset `owner_id IS NULL` cross-tenant leak | `poker/repositories/prompt_preset_repository.py:132-141` | Query `WHERE owner_id = ? OR owner_id IS NULL OR is_system = TRUE` exposes admin-created or manually-seeded non-system presets to every authenticated user. The `is_system = TRUE` clause already covers system presets correctly; the `OR owner_id IS NULL` is overbroad. | |
| T2-56 | Bare `sqlite3.connect()` sites bypass `busy_timeout` | `flask_app/services/ai_debug_service.py:31, 85`, `flask_app/routes/experiment_routes.py:450, 489, 2443, 2945` | Six raw `sqlite3.connect(persistence_db_path)` call sites bypass `busy_timeout=5000` from `BaseRepository`. Under concurrent load these fail immediately with `OperationalError: database is locked` instead of waiting. | |
...
| T2-58 | `delete_game` leaves orphan rows in 4 tables | `poker/repositories/game_repository.py:265-275` | Deletes from 4 tables but skips `tournament_tracker`, `pressure_events`, `emotional_state`, `controller_state`. Dedicated `delete_*_for_game` methods exist for the last two but are never called. No FK cascade because `PRAGMA foreign_keys=ON` not set. | |
| T2-59 | `SchemaManager` migration connections lack `busy_timeout` | `poker/repositories/schema_manager.py:68-70, 1095-1109` | Has its own `_get_connection()` not extending `BaseRepository`. `_init_db()` and migrations use plain `sqlite3.connect`. If a Flask worker holds a WAL write lock at startup, migrations fail immediately rather than waiting. | |
...
| T3-61 | `on_join` socket handler blocks admin users | `flask_app/routes/game_routes.py:1514` | Checks `user.get('id') != owner_id` with no admin bypass, unlike `send_message` and `progress_game` which allow `owner OR admin`. Admins can't spectate user games via socket. | |
| T3-62 | `_EXPLOITATION_RULE_ORDER` duplicates `rule_order` | `poker/tiered_bot_controller.py:117-125`, `poker/strategy/exploitation.py:978-987` | Two separate definitions; already drifted. Single source of truth in `exploitation.py` would prevent future drift. | |
| T3-63 | Misleading comment in `apply_emotional_window_shift` | `poker/bounded_options.py:1282` | Comment says "For moderate+ severity: if at cap, remove from opposite end" but code only does this for `extreme`. Contradicts the (correct) code behavior. | |
| T3-64 | `_option_spectrum_position` can overflow `all_in` sentinel | `poker/bounded_options.py:980` | Raise position is `3 + raise_to`; all_in is 100000. Deep-stack raises >= 99997 can exceed all_in sentinel, breaking emotional drop ordering. | |
| T3-65 | Duplicate `rd` assignment | `poker/hybrid_ai_controller.py:492, 496` | `rd = range_data or {}` repeated. Copy-paste artifact, harmless. | |
| T3-66 | `detect_fold_events` / `detect_chat_events` are dead code | `poker/pressure_detector.py:197-260` | Neither method invoked by active code path. `detect_fold_events` tested in isolation only. Orphaned surfaces. | |
| T3-67 | `GRAVITY_STRENGTH` config loaded but never consumed | `poker/zone_config.py:55-57` | Config parsed but `get_zone_param('GRAVITY_STRENGTH')` called nowhere. | |
| T3-68 | `commentary_generator.generate_quick_reaction` uses global random | `poker/memory/commentary_generator.py:585` | `random.choice(reactions)` on global module. CLAUDE.md violation. | |
| T3-69 | `needs_llm_normalization` false-positive on lone `*` | `poker/response_validator.py:50-56` | Single-asterisk beat sent to fast-tier LLM cleanup unnecessarily. Minor cost. | |
...
| T3-73 | `RAISE_LEVEL_ACTIONS` missing 2-bet entry | `poker/controllers.py:115-118` | Maps `0: 'open_raise'`, `1: '3bet'`, falls back to `'4bet+'`. Two prior raises maps to `4bet+` not `4bet`. Minor labeling imprecision, not correctness. | |
...
**Test status:** 3678 passed, 8 failed (60.39% coverage). TypeScript clean. Failures cluster around the T1-39 admin-gate test mocking issue + 2 pre-existing flakes + 1 personality determinism regression.
