...
| T3-17 | Schema version hardcoded, no migrations | `poker/repositories/schema_manager.py` | `SCHEMA_VERSION = 148` (as of 2026-06-03) with manual migration methods. No Alembic or equivalent. | **PLAN (2026-06-03)** — consolidation strategy in [`docs/plans/SCHEMA_BASELINE_PLAN.md`](/docs/plans/SCHEMA_BASELINE_PLAN.md): squash the v1..v148 chain at the prod cutover, gated by `tests/test_schema_consistency.py`. Adopting a real framework (Alembic) is deliberately deferred to *after* the squash, not bundled with it. |
...
| T3-41 | Split `AIPlayerController` god object | `poker/controllers.py` (2,809 lines as of 2026-06-03; was ~1,794 at first triage) | 7 responsibilities: LLM, prompts, memory, analysis, resilience, evaluation, normalization. Extract into focused service classes. *(Demoted from T2-10)* | |
| T3-42 | Clean up `usePokerGame` hook | `react/src/hooks/usePokerGame.ts` | 1,237-line hook (661 at first triage) with tightly coupled concerns. Splitting into separate hooks creates ref sync issues. Better approach: extract pure functions (message dedup, avatar caching), organize socket handlers into named setup functions. *(Demoted from T2-11)* | |
| T3-43 | `experiment_routes.py` god route file | `flask_app/routes/experiment_routes.py` | 3,382 lines (3,044 at first triage), 25 endpoints, 42+ functions mixing 5+ concerns: experiment lifecycle, AI assistant chat, live monitoring, config validation, background thread management, SQL tool execution. Split into `experiment_routes.py` (CRUD), `experiment_lifecycle_routes.py`, `experiment_assistant_routes.py`, `experiment_monitoring_routes.py`. | |
| T3-44 | `schema_manager.py` monolith | `poker/repositories/schema_manager.py` | ~7,840 lines (as of 2026-06-03; was 2,949 at first triage — still growing). Single `_init_db()` contains CREATE TABLE for all tables plus the full migration chain to v148 (most migration methods are no-ops). Schema and migrations tightly coupled. Extends T3-17. Split schema definitions by domain and archive historical migrations. | **PLAN (2026-06-03)** — ~5,000 of those lines are the 148 `_migrate_vN_*` methods. They're **load-bearing**: `tests/test_schema_consistency.py` (the squash-precondition gate, currently `xfail`) proves `_init_db()` has drifted from the chain — ~19 tables, ~41 indexes, and 12 table shapes exist only via migrations (cash/ledger/presence/stakes/prestige/coach/avatars were wired migration-only). Strategy to reconcile `_init_db` → squash at the prod cutover → archive the chain: [`docs/plans/SCHEMA_BASELINE_PLAN.md`](/docs/plans/SCHEMA_BASELINE_PLAN.md). |
| T3-45 | `run_ai_tournament.py` god script | `experiments/run_ai_tournament.py` | 3,063 lines (2,513 at first triage), 13 classes. Single file contains dataclass definitions, rate limiting, worker thread management, game simulation loop, pause/resume coordination, AI interpretation, CLI parsing. `AITournamentRunner` alone is ~1,200 lines. Split into config, runner, worker, simulator, and CLI modules. | |
| T3-46 | `admin_dashboard_routes.py` oversized | `flask_app/routes/admin_dashboard_routes.py` | 2,067 lines (1,794 at first triage), 34 endpoints mixing admin UI redirects, analytics, model management, data export, and complex SQL query builders (~400 lines). Move analytics to dedicated service layer; extract model management to separate route file. | |
| T3-47 | `game_handler.py` mixed responsibilities | `flask_app/handlers/game_handler.py` | **4,634 lines** (1,465 at first triage — ~3× growth, split never done), 21+ top-level functions. Named "handler" but contains game loop logic, avatar reactions, pressure detection, tournament completion, AI commentary generation. **Hotspot**: `handle_evaluating_hand_phase()` is now ~545 lines (215 at first triage; see MA-17) doing 14+ operations (winner determination, pot award, showdown prep, async commentary spawn, pressure events, memory update, coaching progression, eliminations, tournament check, psychology recovery, new hand setup, guest tracking). `progress_game()` is 165 lines with ~100 lines of run-it-out logic (nested conditionals, sleep delays, reaction scheduling). **Split into**: `game_loop.py` (progress_game, phase transitions), `hand_completion.py` (handle_evaluating_hand_phase, showdown, winners), `ai_action_handler.py` (handle_ai_action, decision execution), `tournament_handler.py` (eliminations, completion), `runout_handler.py` (run-it-out reveals, reaction scheduling), `commentary_handler.py` (generate_ai_commentary, memory feeding). | |
...
| ID | Issue | Location | Description | Status |
|----|-------|----------|-------------|--------|
| T3-48 | `DecisionAnalyzer.tsx` oversized component | `react/react/src/components/admin/DecisionAnalyzer/DecisionAnalyzer.tsx` | 2,485 lines (2,026 at first triage) — among the largest React components (now 2nd to PersonalityManager). Single component handles list view, detail view, replay mode, interrogation mode, JSON export, pagination. 50+ useState hooks, 30+ useEffect hooks. Split into `DecisionList`, `DecisionDetail`, `ReplayMode`, `InterrogationMode` with extracted hooks. | |
| T3-49 | `PersonalityManager.tsx` oversized component | `react/react/src/components/admin/PersonalityManager.tsx` | **3,225 lines** (1,989 at first triage — now the largest React component) with 16 inline sub-components. Mixes personality CRUD, trait editing with elasticity sliders, avatar image management, verbal/physical tics editor, collapsible section logic (duplicated 5+ times), mobile layouts. 20+ useState hooks. Extract sub-components into separate files. | |
| T3-50 | `UnifiedSettings.tsx` settings overload | `react/react/src/components/admin/UnifiedSettings.tsx` | 1,500 lines (1,252 at first triage) managing 5 settings categories (models, capture, storage, pricing, appearance) in one component. Category switching intertwined with data fetching. Extract each category into its own component, keep `UnifiedSettings` as thin switcher (~200 lines). | |
| T3-51 | `ConfigPreview.tsx` kitchen sink | `react/react/src/components/admin/ExperimentDesigner/ConfigPreview.tsx` | 1,293 lines (1,148 at first triage) handling form view, JSON view, config validation, version history, personality selection, prompt preset loading, seed word generation, launch logic. Split into `ConfigFormView`, `ConfigJsonView`, and extracted hooks for validation/versions/seeds. | |
...
| T3-75 | `CharacterDetailCard.tsx` oversized component | `react/react/src/components/character/CharacterDetailCard.tsx` | **1,518 lines** (untracked until 2026-06-03 — 3rd-largest frontend file). A single "dossier" card mixing profile display, AFFILIATIONS, BEHAVIORAL INDEX, FIELD STANDING/NOTES, DEEP READ, observed-remark feed, and an inline "Send chat" surface (9 `useState`, 7 `useEffect`). Split the dossier sections into sub-components and lift the chat surface out. | |
| T3-76 | `ExperimentDetail.tsx` oversized component | `react/react/src/components/admin/ExperimentDesigner/ExperimentDetail.tsx` | 1,274 lines (untracked until 2026-06-03), 17 `useState`. Mixes Configuration view, live Progress/monitoring, per-Decision analysis, By-Player breakdown, Per-Hand and Menu-Compliance/Avg-EV-Lost analytics in one component. Split by view (config / progress / analytics). | |
| T3-77 | `ChipLedgerPanel.tsx` oversized component | `react/react/src/components/admin/ChipLedgerPanel.tsx` | 1,105 lines (untracked until 2026-06-03), 15 `useState`. Cash-economy admin mixing Ledger view, reconciliation "Actual view", Bank-pool-flow analytics, By-reason (all-time/24h) aggregations, and loan/seat readouts. Extract per-view panels. | |
| T3-78 | `PricingManager.tsx` oversized component | `react/react/src/components/admin/PricingManager.tsx` | 1,097 lines (untracked until 2026-06-03), 20 `useState`, 6 `useEffect`. Model-pricing CRUD mixed with provider/model filtering, enabled-model toggles, and validity-date management. Extract the add/edit form and the filter/list into sub-components. | |
...
| T3-70 | DecisionAnalyzer god-component growth (worsens T3-48) | `react/react/src/components/admin/DecisionAnalyzer/DecisionAnalyzer.tsx` | ~2,485 lines (was ~1,800), detail panel duplicated for mobile (~986) and desktop (~1438). Bug fixes must be applied twice. | |
...
| T3-79 | Tournament unification step 3: collapse `TournamentTracker` into `TournamentSession` | `poker/tournament_tracker.py` (deleted), `flask_app/handlers/game_handler.py`, `flask_app/handlers/single_table_tournament.py`, `flask_app/handlers/tournament_completion.py`, `flask_app/routes/game_routes.py` | **DONE & verified** (branch `tournaments`, steps 1–3C). Every game is now a `TournamentSession` (single = 1-table); one unified completion path/screen/career-stats, one persistence + cold-load path. `TournamentTracker` + `handle_eliminations`/`check_tournament_complete` deleted; cold-load migrates legacy saved-tracker blobs into sessions; cash isolation is structural (no session ⇒ no tournament completion). Design + progress: [`docs/plans/TOURNAMENT_UNIFICATION_STEP3.md`](/docs/plans/TOURNAMENT_UNIFICATION_STEP3.md). **FIXED** | Follow-up (separate): the `tournament_tracker` DB table + `load_tournament_tracker` are retained read-only for legacy migration — drop once no legacy games remain. |
| T3-81 | Live tables seat personas at baseline mood; live games never flush mood back | `flask_app/routes/cash_routes.py:622` (`_build_cash_game`), `flask_app/routes/cash_routes.py:4459` (`_leave_table_locked`), `flask_app/handlers/tournament_game_builder.py`, `cash_mode/full_sim.py:427,501` | A persona's emotional state persists per persona in `ai_bankroll_state.emotional_state_json` (v97) and evolves off-screen, but a live cash/tournament seat builds its controller at *baseline* anchors (no hydrate) and the live game *never* writes mood changes back (`save_emotional_state_json` exists only in `cash_mode/`, never `flask_app/`). So the persisted mood feeds only the lobby card + off-screen sim; the felt is disconnected both ways — a lobby-tilted persona sits down calm, and a session that tilts a persona is lost to the world. Fix: promote the sim's `_hydrate_psychology`/`_flush_psychology` to a shared hook; hydrate on build + flush on leave/settle. Continuity follows the **cash world**, not the table type: cash tables AND cash-world (Circuit) tournaments persist two-way; non-cash tournaments start at baseline and persist nothing. Gate on the economy binding, not the `cash_mode` chip flag. Cold-load must not hydrate (D1). Scope: [`docs/triage/PERSONA_PSYCHOLOGY_HYDRATION_HOOK.md`](/docs/triage/PERSONA_PSYCHOLOGY_HYDRATION_HOOK.md). | **FIXED** (2026-06-03) — shared `cash_mode/psychology_persistence.py` hook; live cash two-way (`_build_cash_game` hydrate + `_leave_table_locked` flush); cash-world tournament two-way (builder hydrate gated on `real_persona_ids` + `finalize_tournament` flush gated on `tournament_is_persona_field`). 8 hook + 2 completion tests; cash 343 / tournament 287 green. All three follow-on refinements also closed: mid-tournament balanced-in re-hydrate (`reconcile_live_table`, +3 tests), per-vacate cash flush (`_remove_departed_ais_from_game`, +2 tests), and idle decay-on-read at hydrate (`project_idle_energy` reused, +3 tests). No deferred items remain. |
| T3-80 | Tournament vs cash seat-identity model divergence | `flask_app/handlers/tournament_game_builder.py:154-159,231-236`, `flask_app/handlers/tournament_handler.py`, `flask_app/handlers/single_table_tournament.py:51`, `tournament/session.py`, `tournament/field.py`, `tournament/identity.py`, `react/react/src/components/tournament/TournamentStandings.tsx` | Tournament seats overload `Player.name` to carry the raw `personality_id` (human: `human:<owner>`) and bolt the friendly label onto `Player.nickname`; cash names the seat by display name and carries `personality_id` out-of-band. The split leaks raw ids to the UI (felt showed `human:guest_jeff`; dossier flashed `james_bond`) and forces a `resolve_display_name` shim + per-surface nickname fallbacks cash doesn't need. **Interim fix shipped**: cold-load now restores `nickname` (`serialization.py`) and the dossier seeds its title from `nickname` (`dossierFromPlayer.ts`). **Remaining**: (a) standings panel still prints `player_id` (backend `standings_view` emits no names); (b) proper fix = add explicit `Player.personality_id`, make `name` the display name in both modes, key all bridges on the id. ~30-35 coupling points; display-name collision risk (R1). Follow-on to T3-79. Scope: [`docs/triage/TOURNAMENT_SEAT_IDENTITY_MODEL.md`](/docs/triage/TOURNAMENT_SEAT_IDENTITY_MODEL.md). | **OPEN** — interim UI leak patched; standings fix + identity unification scoped |
...
*2026-06-03: refreshed god-file line counts (T3-41/42/43/45/46/47/48/49/50/51, T3-70) — several had drifted 2-3× since first triage (`game_handler.py` 1,465→4,634; `PersonalityManager.tsx` 1,989→3,225; `controllers.py` ~1,794→2,809). Current top god-files by line count: `schema_manager.py` 7,840 (fat-by-nature: DDL + v1→v148 migrations), `cash_routes.py` 6,530 (MA-16), `lobby.py` 5,018 (T2-75), `game_handler.py` 4,634 (T3-47), `tiered_bot_controller.py` 4,348 (MA-12).*

*2026-06-03: frontend god-component sweep — added T3-75..T3-78 for four untracked components ≥1,000 lines (`CharacterDetailCard.tsx` 1,518, `ExperimentDetail.tsx` 1,274, `ChipLedgerPanel.tsx` 1,105, `PricingManager.tsx` 1,097); corrected T3-42 `usePokerGame.ts` ~1,176→1,237. **Watch-list (900–1,000 lines, not yet tracked):** `PromptDebugger.tsx` 954, `PokerTable.tsx` 952, `PromptPlayground.tsx` 914 — promote to entries if they cross ~1,200.*

*2026-06-03 (tournaments→development merge): added the tournament-unification (**T3-79**), seat-identity (**T3-80**), and persona-psychology-hydration (**T3-81**) items. Renumbered from on-branch T3-75/76/77 to the next free slots — `development` had already claimed T3-75..T3-78 for the frontend god-component sweep above. (Commit messages / captain's logs on the `tournaments` branch still reference the old 75/76/77 — historical.)*
...
| LC-05 | Feature-flag terminality | **Verified by deep sweep (2026-06-03): 11 of 16 flags have a dead branch.** Always-ON (else branch dead): `COMMENTARY_ENABLED`, `DOSSIER_SCOUTING_GATE_ENABLED`, `PRESENCE_AUTHORITY_ENABLED`, `RAKE_ENABLED`, `REPUTATION_DEMEANOR_ENABLED`, `SARCASM_DETECTION_ENABLED`, `SIDE_HUSTLE_ENABLED`. Always-OFF (then branch dead): `GUEST_FREE_CHAT_ENABLED`, `PRESENCE_SHADOW_WRITE_ENABLED`, `PRESTIGE_SEEKING_ENABLED`, `REGEN_ENABLED`. Genuinely active (keep): `CHIP_CUSTODY_ENABLED`, `CSRF_PROTECTION_ENABLED`, `GUEST_LIMITS_ENABLED`, `MODERATION_ENABLED`, `WORLD_TICKER_ENABLED` | Each terminal flag = one dead branch + the flag to remove. Re-confirm the prod pin before deleting — several (cash/economy, sarcasm) may be intentionally left toggleable even if currently terminal. |
...
| LC-07 | Frontend unused exports | **Deep sweep triaged the 249 ts-prune candidates → 136 confirmed dead** (113 were barrel re-exports / dynamic / test-only false positives). Confirmed-dead by area: `misc` 66, `components/admin` 32, `components/debug` 32, `components/settings` 3, `components/chat` 2, `components/stats` 1. Incl. truly-unused hooks (`useGameState`, `useMediaQuery`, `usePolling`, `useSocket`) + `config/timing.ts` consts. **Full 136-export list: [`docs/triage/LEGACY_DEEP_SWEEP_2026-06-03.md`](/docs/triage/LEGACY_DEEP_SWEEP_2026-06-03.md).** | Safe-ish deletions (no real importer). Frontend `depcheck`/`knip` (unused npm deps/files) still deferred — didn't run cleanly in-container. |
...
| LC-09 | **Dead Flask routes** (deep sweep, 2026-06-03) | **24 of 231 routes have zero callers** (verified: no frontend fetch, no internal caller/`url_for`, not health/webhook/external). Incl. `GET /admin/api/summary`, `POST /admin/api/playground/cleanup`, `POST /api/cash/stakes/<id>/default` (built-unwired "Phase 2 Commit 2"), `GET /api/coach/metrics/{overview,skills,advancement}`, several `/api/experiments/*` (cost-trends, regenerate-summary, set-type, trajectory-viewer), `GET /api/personality/<name>`, 4× `/api/prompt-debug/*`, `GET /api/game/<id>/decision-quality`, `GET /api/models`, `GET /api/groups`, `GET /settings/<id>`. **Full list + per-route evidence: [`docs/triage/LEGACY_DEEP_SWEEP_2026-06-03.md`](/docs/triage/LEGACY_DEEP_SWEEP_2026-06-03.md).** | **Verify intent before deleting** — `/api/test/*` (snapshot/reset) are test-only and likely keep; some may be near-term-planned API surface. 18 more are `uncertain` (admin SSR pages, retry, llm-configs) — not dead, just no static caller found. |
