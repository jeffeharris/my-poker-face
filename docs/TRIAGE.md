...
| T2-53 | Coach `effective_stack`/`SPR` regress `test_owner_can_act_on_human_turn` | `flask_app/services/coach_engine.py`, `tests/test_websocket_auth.py:236` | The new coach context fields use `compute_spr` and stack arithmetic that fail with `'<' not supported between MagicMock and int`. Test mocks insufficient for the post-merge computation. | |
