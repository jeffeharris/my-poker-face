...
<!-- 2026-06-08: the four archetype width-tier charts (loose/loose_mid/station/weak_station) AND tight_rfi are GENERATED by experiments/build_archetype_charts.py from the base 100bb chart — corrected provenance + added the generator row. tight_rfi gained a premium-3bet damp (build_tight). -->
...
| `preflop_100bb_6max_tight_rfi.json` | 8,450 | snapshot 2026-05-27; **regenerated 2026-06-08** by `build_archetype_charts.build_tight` | Tight RFI (the pre-widening opens, preserved verbatim) over the base facing rows with their premium re-raise mass damped into flat-calls (`damp_raise`). The `nit`/`rock` width-tier table; also the "tighten-vs-station" table (`EXP_003`). Before the damp its facing rows were a verbatim base copy, so nit/rock inherited the base's ~14.5% facing-open 3-bet (over band) — see `ARCHETYPE_SHAPING_FINDINGS.md` (Knob 2) | `preflop_100bb_6max_README.md` (log) |
| `preflop_100bb_6max_loose.json` | 8,450 | 2026-05-29 | **Generated** by `experiments/build_archetype_charts.build_loose` — widest realistic opening envelope; the `maniac` / `maniac_overbluff` / `spewy_fish` preflop table. Loose RFI sets + `_loosen_facing` over the base 100bb taxonomy | `preflop_100bb_6max_README.md` (log) |
| `preflop_100bb_6max_loose_mid.json` | 8,450 | 2026-05-29 | **Generated** by `build_archetype_charts.build_loose_mid` — between TAG and Maniac; the `lag` preflop table. Low `raise_share` so it flats wide instead of 3-betting wide (PR #240 trim) | `preflop_100bb_6max_README.md` (log) |
| `preflop_100bb_6max_station.json` | 8,450 | 2026-05-29; **damp added 2026-06-08** | **Generated** by `build_archetype_charts.build_station` — loose-passive caller shape; the `calling_station` (+ `_pblind`/`_overbluff` isolation variants) table. `_station_facing` floods fold→call AND now damps the base's premium re-raise mass into call (`damp_raise=0.85`) so the station *traps* premiums rather than 3-betting them (facing-open 3-bet ~18%→~6% chart, ~3% realized) | `preflop_100bb_6max_README.md` (log) |
| `preflop_100bb_6max_weak_station.json` | 8,450 | 2026-05-29; **damp added 2026-06-08** | **Generated** by `build_archetype_charts.build_weak_station` — widest passive-caller shape (flats almost anything vs a raise); the `weak_fish` ($2-tier trickle) table. Same premium-trap damp (`damp_raise=0.80`). NOT reachable via anchor classification — explicit loadout. See `FISH_AS_CALLING_STATION.md` | `preflop_100bb_6max_README.md` (log) |
...
| `experiments/build_archetype_charts.py` | Generator | No | Produces the five archetype width-tier charts — `tight_rfi` (`build_tight`), `loose` (`build_loose`), `loose_mid` (`build_loose_mid`), `station` (`build_station`), `weak_station` (`build_weak_station`) — from `preflop_100bb_6max.json`. Transforms: `_loosen_facing` (cut fold, split freed mass call/raise via `raise_share`), `_station_facing` (flood fold→call + `damp_raise` to trap premiums). Lives in `experiments/`, not `poker/strategy/data/`. Re-run: `docker compose exec -T backend python -m experiments.build_archetype_charts`. **Never hand-edit these five JSONs — change the generator + re-run.** Rationale: `docs/technical/ARCHETYPE_SHAPING_FINDINGS.md`, `docs/plans/PERSONALITY_PRICING_AND_VARIETY.md`. |
