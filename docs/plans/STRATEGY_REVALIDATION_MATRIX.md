...
| `_apply_relationship_modifier_to_offsets` | ON (default True); bites only w/ accrued heat | **VALIDATED INERT 2026-06-12** — scales the additive-logit `offsets` channel the re-arch proved can't flip a sampled action. Forced max-rival on every decision (11,886 engagements, 21,494 offsets scaled) → **0.0pp** behavior change. No live EV risk; rebuild on the hard-override channel if relationship exploitation is wanted. | `relationship_modifier_probe.py` (Phase-2 abort) | ~~high~~ **inert** | **done** | relationship_modifier_probe |
...
2. ~~**`_apply_relationship_modifier_to_offsets` (ON by default, never measured).**~~ **DONE 2026-06-12 — RESOLVED INERT (no bb/100 needed).** Closed at Phase 2 instead of Phase 3: `experiments/relationship_modifier_probe.py` forced the strongest possible modifier (max-rival, aggressive offsets ×1.3) on *every* decision (TAG vs CallStation HU, the matchup where the additive offsets populate 75.9% of decisions) — **11,886 engagements scaling 21,494 offsets → 0.0pp behavior change** (byte-identical CRN decision distribution). The layer scales the additive-logit `offsets` channel the Tier-2 re-arch already proved can't flip a sampled action; it never reaches the hard-override or `value_vs_station`-intensity channels that move behavior. There is **no EV to measure** on the current architecture, so "highest unknown" is downgraded to **inert / no live EV risk**. It also can't be EV-measured *at all* without a seeded-heat bed (`simulate_bb100` hard-disables the flag; `exploit_bb100` has no `_relationship_repo`). If relationship-based exploitation is wanted, rebuild it on the hard-override / gear-switch channel — a soft multiplier on the dead offset channel is a no-op by construction.
