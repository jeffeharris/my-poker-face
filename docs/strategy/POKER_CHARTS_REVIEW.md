...
- The **50bb / 25bb depth charts use the same RFI as 100bb** — opens are passed
  ...
  *widened* 100bb opens (UTG 11.5 … CO 27.3 … BTN 47.5). (An earlier draft claimed
  depth kept the old *tight* opens — that was wrong; verified against the live JSON.)
...
- **RFI is passed through unchanged** — opens are byte-identical to the 100bb chart at every depth (`t_rfi` is identity; the depth logic only rewrites the facing nodes). So shallow RFI carries the **current widened** opens (UTG 11.5 … CO 27.3 … BTN 47.5), the same as 100bb. (A prior version of this note claimed these were the *old tight* pre-widening opens — that was wrong; confirmed identical to the 100bb RFI in the live JSON.)
