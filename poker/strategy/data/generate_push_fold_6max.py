...
def build_chart() -> Dict:
    ...
    for position, by_depth in UNOPENED_RANGES.items():
        ...
        for depth in DEPTH_BUCKETS:
            ...
            chart["unopened"][position][str(depth)] = _build_action_row(
                hand_set, "jam"
            )
    ...
    for table, by_depth in CALL_VS_SHOVE_RANGES.items():
        ...
        for depth in sorted(by_depth.keys()):
            ...
            chart["call_vs_shove"][table][str(depth)] = _build_action_row(
                hand_set, "call"
            )
...
def main() -> int:
    ...
    for position in UNOPENED_RANGES:
        ...
        for depth in DEPTH_BUCKETS:
            ...
            cells.append(
                f"{combo_pct(hs):>5.1f}/{UNOPENED_TARGET_PCT[position][depth]:<4.0f}"
            )
    ...
    for table, by_depth in CALL_VS_SHOVE_RANGES.items():
        ...
        for depth in sorted(by_depth.keys()):
            hs = trim_to_target(
                expand_range(by_depth[depth]), CALL_TARGET_PCT[table][depth]
            )
