...
def load_archetype_preflop_tables() -> Dict[str, "StrategyTable"]:
    """Load the width-tier preflop charts keyed by deviation-profile key.

    Returns a dict like ``{'lag': StrategyTable, 'maniac': ..., 'nit': ...,
    'rock': ..., 'calling_station': ...}`` for every archetype whose
    ARCHETYPE_WIDTH_TABLE entry names an existing file. Archetypes mapped to
    ``None`` (e.g. ``tag``) use the standard base chart and are absent here, as
    is any whose file is missing — callers fall back to the base table. These
    are **preflop-only** (postflop stays on the base chart's distortion path),
    so they carry only preflop_data, exactly like the depth/HU tables.

    The loose/station charts are generated by
    ``experiments/build_archetype_charts.py``; tight_rfi predates this. See
    docs/plans/PERSONALITY_PRICING_AND_VARIETY.md for why tables (not just
    distortion) carry the coarse VPIP envelope.
    """
    from .deviation_profiles import ARCHETYPE_WIDTH_TABLE

    data_dir = os.path.join(os.path.dirname(__file__), 'data')
    tables: Dict[str, StrategyTable] = {}
    for key, filename in ARCHETYPE_WIDTH_TABLE.items():
        if not filename:
            continue
        ...
        with open(path) as f:
            preflop_raw = json.load(f)
        tables[key] = StrategyTable(_parse_json_to_preflop_data(preflop_raw), postflop_data={})
    return tables
