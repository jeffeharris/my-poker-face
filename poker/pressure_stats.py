...
class PressureStatsTracker:
    ...
    def record_event(
        self, event_type: str, player_names: List[str], details: Optional[Dict[str, Any]] = None
    ) -> None:
        ...
        for player_name in player_names:
            ...
            if 'pot_size' in details:
                self.game_stats['biggest_pot'] = max(
                    self.game_stats['biggest_pot'], details['pot_size']
                )
        ...
        return {
            ...
            'master_bluffers': sorted(
                [
                    ...
                    for p in players
                    if p.successful_bluffs > 0
                ],
                key=lambda x: x['bluffs'],
                reverse=True,
            )[:3],
            'most_aggressive': sorted(
                [
                    {
                        'name': p.player_name,
                        'aggression': p.get_aggression_score(),
                        'aggressive_bets': p.aggressive_bets,
                    }
                    for p in players
                ],
                key=lambda x: x['aggression'],
                reverse=True,
            )[:3],
            'bad_beat_victims': sorted(
                [
                    ...
                    for p in players
                    if p.bad_beats_suffered > 0
                ],
                key=lambda x: x['bad_beats'],
                reverse=True,
            )[:3],
            'tilt_masters': sorted(
                [
                    ...
                    for p in players
                    if p.get_tilt_score() > 0
                ],
                key=lambda x: x['tilt_score'],
                reverse=True,
            )[:3],
        }
    ...
    def get_fun_facts(self) -> List[str]:
        ...
        for player_name, stats in self.player_stats.items():
            if stats.successful_bluffs >= 3:
                facts.append(
                    f"🎭 {player_name} is a master of deception with {stats.successful_bluffs} successful bluffs!"
                )

            if stats.bad_beats_suffered >= 2:
                facts.append(
                    f"💔 {player_name} has suffered {stats.bad_beats_suffered} bad beats. Ouch!"
                )
            ...
            if stats.get_tilt_score() > 0.7:
                facts.append(
                    f"😤 {player_name} is on major tilt with a {stats.get_tilt_score():.0%} tilt score!"
                )
        ...
        if self.game_stats['total_events'] > 20:
            facts.append(
                f"🎢 What a wild game! {self.game_stats['total_events']} dramatic moments so far!"
            )
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'player_summaries': {
                name: stats.get_summary() for name, stats in self.player_stats.items()
            },
        ...
        try:
            ...
            for event_entity in events:
                ...
                if 'pot_size' in event.details:
                    self.game_stats['biggest_pot'] = max(
                        self.game_stats['biggest_pot'], event.details['pot_size']
                    )
