...
class GamePersistence:
    ...
    def _run_migrations(self) -> None:
        ...
        migrations: Dict[int, tuple] = {
            ...
            63: (self._migrate_v63_add_coach_progression, "Add coach progression tables and can_access_coach permission"),
        }
    ...
    # ------------------------------------------------------------------
    # Migration v63: Coach progression tables
    # ------------------------------------------------------------------

    def _migrate_v63_add_coach_progression(self, conn: sqlite3.Connection) -> None:
        """Migration v63: Add coach progression tables and can_access_coach permission."""

        # player_skill_progress — per-user, per-skill tracking
        conn.execute("""
        ...
        """)
        logger.info("Created player_skill_progress table")

        # player_gate_progress — per-user gate unlock tracking
        conn.execute("""
        ...
        """)
        logger.info("Created player_gate_progress table")

        # player_coach_profile — per-user coaching profile
        conn.execute("""
        ...
        """)
        logger.info("Created player_coach_profile table")

        # Insert can_access_coach permission and assign to user + admin groups
        conn.execute("""
            INSERT OR IGNORE INTO permissions (name, description, category)
            VALUES ('can_access_coach', 'Access to the AI coaching progression system', 'coach')
        """)
        ...
        conn.execute("""
            ...
            SELECT g.id, p.id
            FROM groups g, permissions p
            WHERE g.name = 'admin' AND p.name = 'can_access_coach'
        """)
        # Grant to user group
        conn.execute("""
            INSERT OR IGNORE INTO group_permissions (group_id, permission_id)
            SELECT g.id, p.id
            FROM groups g, permissions p
            WHERE g.name = 'user' AND p.name = 'can_access_coach'
        """)

        logger.info("Migration v63 complete: coach progression tables and permission added")

    # ------------------------------------------------------------------
    # Coach progression CRUD
    # ------------------------------------------------------------------
    ...
    def load_gate_progress(self, user_id: str):
        ...
        from flask_app.services.skill_definitions import GateProgress
        with self._get_connection() as conn:
            cursor = conn.execute(
