...
class TournamentRepository(BaseRepository):
    ...
    def update_career_stats(
        self, owner_id: str, player_name: str, tournament_result: Dict[str, Any]
    ) -> None:
        ...
        player_standing = next((s for s in standings if s.get('player_name') == player_name), None)
        ...
        eliminations_this_game = sum(1 for s in standings if s.get('eliminated_by') == player_name)
    ...
    def get_career_stats(self, owner_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self._get_connection() as conn:
            ...
            return {
                ...
                'win_rate': row['games_won'] / row['games_played']
