...
class HandEvaluator:
    ...
    def evaluate_hand(self):
        ...
        for i, check in enumerate(checks, start=1):
            ...
            if result[0]:
                return {
                    "hand_rank": i,
                    "hand_values": result[1],
                    "kicker_values": result[2],
                    "suit": result[3],
                    "hand_name": result[4],
                }
        return {
            "hand_rank": 10,
            "hand_values": [],
            "kicker_values": sorted(self.ranks, reverse=True)[:5],
            "hand_name": "High Card",
        }

    def _check_royal_flush(self):
        has_straight_flush, straight_flush_values, _, straight_flush_suit, _ = (
            self._check_straight_flush()
        )
        if has_straight_flush:
            ...
            if straight_flush_values == comparison_set:
                return (
                    ...
                    [],
                    straight_flush_suit,
                    f"Royal Flush with {straight_flush_suit}",
                )
        ...
        if has_flush:
            ...
            if has_straight:
                return (
                    ...
                    [],
                    flush_suit,
                    f"{rank_to_display(straight_values[0])} high Straight Flush with {flush_suit}",
                )
        return False, [], [], None, None
    ...
    def _check_full_house(self):
        ...
        if three is not None and two is not None:
            return (
                ...
                [],
                None,
                f"Full House {rank_to_display(three)}'s over {rank_to_display(two)}'s",
            )
        return False, [], [], None, None
    ...
    def _check_flush(self):
        for suit, count in self.suit_counts.items():
            if count >= 5:
                flush_cards = sorted(
                    [card.value for card in self.cards if card.suit == suit], reverse=True
                )
        ...
        for rank, count in self.rank_counts.items():
            if count == 3:
                ...
                return (
                    ...
                    kickers,
                    None,
                    f"Three of a kind with {rank_to_display(rank)}'s",
                )
        return False, [], [], None, None
    ...
    def _check_two_pair(self):
        ...
        if len(pairs) >= 2:
            ...
            return (
                ...
                kickers,
                None,
                f"Two Pair, {rank_to_display(pairs[0])}'s and {rank_to_display(pairs[1])}'s",
            )
        return False, [], [], None, None
