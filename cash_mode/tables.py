...
def ai_slot_ephemeral(profile, chips: int) -> Dict[str, Any]:
    """Return an AI slot dict for an ephemeral tourist.

    Sibling to `ai_slot` for tourists generated by
    `cash_mode.tourist_factory.generate_tourist`. Carries the inline
    personality dict so seat→personality lookups don't have to hit
    `PersonalityRepository` for ephemeral seats (those rows don't
    exist in `personalities`).

    `profile` is a `TouristProfile`; typed loosely to avoid an import
    cycle (tables.py is lower-level than tourist_factory.py).
    """
    return {
        "kind": "ai",
...
def personality_for_seat(seat: Dict[str, Any], personality_repo) -> Optional[Dict[str, Any]]:
    """Resolve a seat to its personality config dict.

    Inline `ephemeral_personality` (set by `ai_slot_ephemeral`) wins
    over a DB lookup — ephemeral tourists don't have `personalities`
    rows. For regular AI seats, falls through to
    `PersonalityRepository.load_personality_by_id`. Returns None for
    open/human seats or when the lookup fails.
